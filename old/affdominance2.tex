...
Write $\alpha$ as $\beta+\gamma$ such that $\beta$ is a linear combination of simple roots in non-affine entries and $\gamma$ is a linear combination of simple roots in affine positions.
Let $B'$ be the $n\times(n-2)$ matrix obtained from the non-affine columns of $B$ by setting the entries in affine rows to $0$.
Comparing \cref{neigh good stuff}.\ref{neigh im ray} with \cref{neigh B} (and especially \cref{submat tab}), we see that $B\beta$ can be obtained as a nonnegative linear combination of the columns of $B'$ and the vector $-\frac12B\delta^B$.
\cref{delta is the man} says that $B\beta$ is contained in $(\delta^B)^\perp$.
Also, $B'$ and $\Comp(B)$ have the same nonnegative linear span, so $B\beta$ is 
...
FINISH REWRITING THIS DIRECTLY!!!
...
\sayN{Fix alignment of all matrices, if necessary using [ and small and array instead of bsmallmatrix.}
