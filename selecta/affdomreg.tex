...
\title{Dominance regions for affine cluster algebras\thanks{Nathan Reading was partially supported by the Simons Foundation under award number 581608 and by the National Science Foundation under award number DMS-2054489. Salvatore Stella was partially supported by PRIN 20223FEA2E, \lq\lq Cluster algebras and Poisson Lie groups\rq\rq and INdAM/GNSAGA}}
\author{Nathan Reading\thanks{Department of Mathematics, North Carolina State University, Raleigh, NC, USA} \and
Dylan Rupel\thanks{Math Academy, Pasadena Unified School District, Pasadena, CA, USA} \and
Salvatore Stella\thanks{Dipartimento di Ingegneria e Scienze dell'Informazione e Matematica, Universit\`{a} degli Studi dell'Aquila, IT}}
%\keywords{}
...
\noindent \textbf{Keywords:} Cluster algebras, Dominance order, Pointed bases.

\noindent \textbf{MSC2020:} Primary 13F60.
\end{abstract}
...
For our applications, the relevant notion is the integral dominance region: the set of points in $\P_{\tilde\lambda}^\tB$ that are $\tilde\lambda$ plus an \emph{integer}-linear combination of columns of $\tB$.
...
The diagonal matrix $\symmetrizer$ with diagonal entries $d_1,\ldots,d_n$ is such that $\symmetrizer B\symmetrizer^{-1}=-B^T$.
...
The map $\eta_{\kk}^{B_0^T}$ is linear on $\Cone_t^{B_0;t_0}$ and it is $\eta_{\kk}^{B_0^T}={\eta_{k_r}^{B_{r-1}^T}\circ\cdots\circ\eta_{k_2}^{B_1^T}\circ\eta_{k_1}^{B_0^T}}$.
...
In light of Lemma~\ref{block dom}, we may assume that $B$ is block-indecomposable.
...
\begin{figure}
  \centering
\scalebox{0.75}{\includegraphics{theti1.ps}
\begin{picture}(0,0)(73,-73)
\put(-21,-17){$\thet_{i_p}$}
...
\end{picture}}
\hspace{0.5pt}
\scalebox{0.75}{\includegraphics{theti1.5.ps}
\begin{picture}(0,0)(73,-73)
\put(2.5,9){\small$L_p$}
\put(-16,-33){$L'_p$}
%\put(5,-19){$t_{i_p}$}
\end{picture}}
\hspace{0.5pt}
\scalebox{0.75}{\includegraphics{theti2.ps}
\begin{picture}(0,0)(73,-73)
...
Let $\Theta^{(\ell)}$ to be the lamination whose shear coordinates are $-\mu_{\kk^\ell}(B)\alpha^{(\ell)}=-B\alpha^{(\ell)}$.
...
For $i>I_+$ or $i<I_-$, $\br{\bigl(\eta_{12\cdots n}^{B^T}\bigr)^i(x),\beta}<0$ for all $\beta\in\RS$ with $\omega_c(\beta,\delta)>0$.
\item \label{c agree}
%\item \label{c agree +}
...
Also, $\mu_{12\cdots n}(B_0)=B_0$, so from Lemma~\ref{shift}.\ref{shift all} we get that ${\bigl(\eta^{B_0^T}_{12\cdots n}\bigr)^\ell(\P_\lambda^{B_0})=P_\lambda^{B_0}}$.
...
Finally, a non-acyclic skew-symmetrizable $3\times3$ exchange matrix having $b_{12}b_{21}=b_{13}b_{31}=b_{23}b_{32}=-4$ has exponential growth.
...
\]
which rules out all pairs $(b_{kp},b_{kn})$ except for $(1,-2)$, $(2,-3)$, and $(2,-4)$.
...
Thus $\CompPlus(\mu_\kk(B))$ agrees in the affine rows with $\CompPlus(B)$.
...
Reasoning as in the previous cases one proves also the assertions about $\CompPlus(\mu_\kk(B))$.
...
The set $\P^B_{\lambda,\widehat\kk}$ is $(\eta_{\widehat\kk}^{B^T})^{-1}\set{\eta_{\widehat\kk}^{B^T}(\lambda)+B'\alpha:\alpha\in\reals^n,\alpha\ge0}$.
...
Given an integer point $\tilde\lambda\in\integers^m$, let $\IP^\tB_{\tilde\lambda,\kk}=\bigl(\eta_{\kk}^{\BB^T}\bigr)^{-1}\sett{\eta_\kk^{\BB^T}(\tilde\lambda)+\mu_\kk(\tB)\alpha:\alpha\in\integers^n,\alpha\ge0}$ for all sequences~$\kk$.
