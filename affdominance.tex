...
If $\lambda$ is in the imaginary wall~$\d^B_\infty$, then the dominance region $\P^B_\lambda$ is the line segment parallel to the imaginary ray, with one endpoint at $\lambda$ and the other endpoint on the relative boundary of $\d^B_\infty$.
\end{theorem}
...
The subspace consisting of vectors $x$ such that $\omega_c(x,\delta)=0$ has dimension $n-1$ and the subspace consisting of vectors in finite $c$-orbits also has dimension $n-1$ (for example, it contains $n-1$ linearly independent roots), so these subspaces must coincide.
\end{proof}
...
\cref{finite P point} is an immediate consequence of the following more specific \lcnamecref{finite P point bip}.
...
(In general, one must be careful of the difference between rational laminations and real laminations, but we will only consider finite type, where one may as well take laminations to have real weights.)
...
Then up to switching the orientation of the plane, $T$ is as exemplified in \cref{T fig}.
\begin{figure}
\scalebox{1}{\includegraphics{Tfig.ps}}
\begin{picture}(0,0)(58,-58)
\put(-46.5,-23){$t_1$}
\put(-36.5,-1){$t_2$}
\put(-19,-26){$t_3$}
\put(-3.5,5){$t_4$}
...
\put(42,-3){$t_7$}
\end{picture}
\caption{A bipartite triangulation $T$}
\label{T fig}
\end{figure}
We refer to the arcs of $T$ as $t_1,\ldots,t_n$.
One easily verifies that applying the mutation $\mu_\kk$ to $B$ corresponds to an $(n+3)$-fold rotation of~$T$.
...
Let $\Lambda$ be the weighted lamination such that $\lambda$ is the negation of the shear coordinates of $\Lambda$.
This choice of $\Lambda$ ensures that reading the negative shear coordinates of $\Lambda$ with respect to a rotation of $T$ (i.e.\ replacing $B$ by  $\mu_{\kk^\ell}(B)$ for some $\ell\in\integers$) gives $\eta^{B^T}_{\kk^\ell}(\lambda)$.
(See \cite[Theorem~12.6]{cats2}, \cite[Theorem~4.3]{unisurface}, and \cite[Section~5]{unisurface}.)
The lamination $\Lambda$ consists of a set of noncrossing curves, each with a real weight and each connecting an edge of the $(n+3)$-gon to a different and non-adjacent edge.  

Maximal sets of such noncrossing curves have cardinality~$n$.
...
If $\Lambda$ has fewer than $n$ curves, we extend it to a set of $n$ noncrossing curves by adjoining additional curves with weight~$0$.
...
Apply the mutation map $\eta^{B^T}_{\kk^\ell}$ to $\lambda$ by rotating $T$ by $\ell$ units counterclockwise.
Then find a lamination whose shear coordinates, with respect to the rotated triangulation, are the sum of $-\mu_{\kk^\ell}(B)\alpha^{(\ell)}$ and the shear coordinates of $\Lambda$.
...
Then take another lamination $\Theta^{(\ell)}$ whose shear coordinates are the negative of the vector $B\alpha^{(\ell)}$ (rather than $\mu_{\kk^\ell}(B)\alpha^{(\ell)}$).
The lamination $\Theta^{(\ell)}$ has $n$ curves, namely, for $i=1,\ldots,n$, the curve $\thet_i$ with weight $b_i^{(\ell)}=a_{i-1}^{(\ell)}+a_{i+1}^{(\ell)}$, intersecting exactly one arc (the $i\th$ arc) in the rotated triangulation, as exemplified in \cref{Theta fig}.
\begin{figure}
\scalebox{1}{\includegraphics{Theta.ps}}
\begin{picture}(0,0)(113,-73)
\put(-86,-12){$\thet_1$}
\put(-63,12){$\thet_2$}
\put(-26,-31){$\thet_3$}
\put(-1,22){$\thet_4$}
...
\label{Theta fig}
\end{figure}
(The curves $\thet_i$ are independent of $\ell$, but the weights $b_i^{(\ell)}$ depend on $\ell$.)
Let $X^{(\ell)}$ be the lamination whose shear coordinates with respect to the unrotated~$T$ are the sum of the shear coordinates of $\Lambda^{(\ell)}$ and the shear coordinates of $\Theta^{(\ell)}$.
Then rotate $X^{(\ell)}$ counterclockwise $\ell$ units, and read shear coordinates with respect to~$T$.
...
The fact that $x\in\bigcap_{\ell\in\integers}\P^B_{\lambda,\kk^\ell}$ implies the following fact, which we refer to below as the \newword{main observation}:
...
Since $a_i^{(\ell)}\ge 0$ for all $i$ and $\ell$, it is enough to show that each $b_i^{(0)}$ is~$0$.
We will consider two cases.

\smallskip

\noindent
\textit{Case 1.}  \sayN{If we end up needing cases in other types (as is likely in Type D), we might need a more specific label to this case.}
$\thet_i$ coincides with a curve $L_j\in\Lambda$.
This case is represented in \cref{theti fig 1}.
(The meaning of the dotted curve marked $L'_j$ is explained below.
The orientation of the arc $t_i\in T$  in its quadrilateral might be opposite.
If so, then the argument given here still works, with a global change in sign.)
\begin{figure}
\scalebox{1}{\includegraphics{theti1.ps}}
\begin{picture}(0,0)(113,-73)
\put(-17.5,-17){$\thet_i$}
\put(1,25){$L_j$}
\put(43,-13){$L'_j$}
...
\label{theti fig 1}
\end{figure}
The sum of the shear coordinates of $\Lambda$ and $\Theta^{(0)}$ is $w_j+b_i^{(0)}$ in position $t_i$, because no curves of $\Lambda$ or $\Theta^{(\ell)}$ contribute to this coordinate except $\thet_i=L_j$.
Therefore, $X^{(0)}$ has the curve $\thet_i$ with weight $w_j+b_i^{(0)}$ (or, if $w_j+b_i^{(0)}=0$, $X^{(0)}$ consists of curves that don't cross $\thet_i$).

There is a unique curve $L'_j$ that does not cross any curve in $\Lambda$ except $L_j$.  \sayN{If I can mark it in \cref{theti fig 1}, mention that here.}
(The situation is analogous to diagonal flips between triangulations.)
There exists $\ell\in\integers$ such that rotating $L'_j$ by $\ell$ units gives a curve $\thet_m$, as exemplified in \cref{theti fig 2}.
\begin{figure}
...
\label{theti fig 2}
\end{figure}
The shear sum of the shear coordinates of $\Lambda^{(\ell)}$ and $\Theta^{(\ell)}$ is $\pm(w_j-b_m^{(\ell)})$ in position~$t_m$, with the sign determined by the orientation of $t_m$ in its quadrilateral.
The main observation implies that the sum of the shear coordinates of $\Lambda^{(\ell)}$ and $\Theta^{(\ell)}$ has the same sign and is at least $w_j+b_i^{(0)}$ in absolute value in position $t_m$.
Therefore, $b_i^{(0)}=b_m^{(\ell)}=0$.

\smallskip

\noindent
\textit{Claim between cases.}
For each $j=1,\ldots,n$, the lamination $X^{(0)}$ has the curve $L_j$ with weight $\ge w_j$.
(Or, if $w_j=0$ and $X^{(\ell)}$ does not have the curve $L_j$, it consists of curves that don't cross $L_j$.)
To see why, notice that there exists $\ell\in\integers$ such that the rotation of $L_j$ counterclockwise $\ell$ units is a curve $\thet_i$.
As in Case 1, we see that $X^{(\ell)}$ has the curve $\thet_i$ with weight $w_j+b_i^{(\ell)}\ge w_j$ (or, if not, $w_j+b_i^{(\ell)}=0$ and $X^{(\ell)}$ consists of curves that don't cross $\thet_i$).
...
\noindent
\textit{Case 2.}
$\thet_i$ does not coincide with a curve $L_j\in\Lambda$.
Since $\Lambda$ is maximal, there are curves in $\Lambda$ that cross $\thet_i$.
The situation is pictured in \cref{theti fig 3}.
\begin{figure}
\scalebox{1}{\includegraphics{theti3.ps}}
\begin{picture}(0,0)(113,-73)
\put(-15,-22){$\thet_i$}
\put(15,-33){$t_i$}
\end{picture}
\caption{Curves in $\Lambda$ cross $\thet_i$}
\label{theti fig 3}
\end{figure}
The claim implies that each of the curves $L_m$ in $\Lambda$ that cross $\thet_i$ also appears in $X^{(0)}$ with weight at lease $w_m$ (or is compatible with every curve in $X^{(0)}$).
Thus the shear coordinate of $X^{(0)}$ at position $t_i$ is at least the sum of these weights $w_m$, and thus $b_i^{(\ell)}=0$.

\medskip

\noindent
\textbf{Type D.}
The outline of the proof in type D is essentially the same as in type A, and for that reason we omit some explanations that will be clear in this case based on explanations in the type-A case.
The major difference in type D is the appearance of tagged triangulations.
Let $T$ be a tagged triangulation of the once-punctured regular $n$-gon whose signed adjacency matrix is the bipartite exchange matrix~$B$, exemplified in \cref{d T fig}.
\begin{figure}
NEED THIS
%\scalebox{1}{\includegraphics{dTfig.ps}}
%\begin{picture}(0,0)(58,-58)
%\put(-46.5,-23){$t_1$}
%\put(-36.5,-1){$t_2$}
%\put(-19,-26){$t_3$}
...
%\put(42,-3){$t_7$}
%\end{picture}
\caption{A bipartite tagged triangulation $T$}
\label{d T fig}
\end{figure}
...
Second, for each side, the two curves starting on that side and spiraling around the puncture in opposite directions \emph{are} allowed to appear together in a quasi-lamination, even though they intersect.
The lamination $\Lambda$ consists of a set of curves, each with a real weight.
Some of the curves connect an edge of the $n$-gon to different edge.
The two edges must be non-adjacent, unless the curve and the two edges enclose the puncture.
The remaining curves have an endpoint on an edge and, at the other end, spiral into the puncture in either direction.
...
In particular, the negative of the shear coordinate vector of $\Lambda$ with respect to a rotation of $T$ is $\eta^{B^T}_{\kk^\ell}(\lambda)$ for an appropriate~$\ell$.
...
But this time, we skip directly to the more convenient description.
...
We define $\Theta^{(\ell)}$ to be the lamination whose shear coordinates are $-B\alpha^{(\ell)}$ (rather than $-\mu_{\kk^\ell}(B)\alpha^{(\ell)}$).

The lamination $\Theta^{(\ell)}$ has $n$ curves, described as follows and exemplified in \cref{d Theta fig}.
...
The weights on these two curves are $b_1^{(\ell)}=b_2^{(\ell)}=a_3$.
The curve $\thet_3$ intersects $t_3$ and has weight $b_3^{(\ell)}=a_1+a_2+a_4$.
For $i=4,\ldots,n$, the curve $\thet_i$ with weight $b_i^{(\ell)}=a_{i-1}^{(\ell)}+a_{i+1}^{(\ell)}$ and intersects~$t_i$.
\begin{figure}
NEED THIS
%\scalebox{1}{\includegraphics{dTheta.ps}}
%\begin{picture}(0,0)(113,-73)
%\put(-86,-12){$\thet_1$}
%\put(-63,12){$\thet_2$}
%\put(-26,-31){$\thet_3$}
