...
THIS NEEDS MORE HYPOTHESES.

Look at the case $B=\begin{bsmallmatrix*}[r]0&1\\-1&0\end{bsmallmatrix*}$, and $\kk=2$ (singleton sequence), where it appears that you need the 2nd coordinate to be negative.

DISSECT THIS PROOF THROUGH THAT EXAMPLE!
...
\begin{corollary}\label{acyc opp}  \sayN{If we leave this here:  We haven't talked about acyclic yet.}
Suppose $B_0$ is an acyclic $n\times n$ exchange matrix, indexed so that $b_{ij}\ge0$ whenever $i<j$.
If the first entry of $\eta^{B_0^T}_{1\cdots n}(\lambda)$ is nonnegative, then 
\[\P^{B_0}_\lambda\subseteq\set{\lambda+B_0\alpha:\alpha\in\reals^n,\alpha\ge0}\cap\set{\lambda-B_0\alpha:\alpha\in\reals^n,\alpha\ge0}.\]
\end{corollary}
\begin{proof}
Let $\kk=1\cdots n$ and take $t_0\overset{n}{\edge}t_1\overset{n-1}{\edge}\,\cdots\,\overset{1}{\edge}t_n=t$ with $B_{t_0}=B_0$.
Easy standard calculations show that $B_t=B_0$, that $\kk^{-1}=1\cdots n$ is a red sequence for $B_0$ and that $C_t^{B_0;t_0}$ is the negative of the identity matrix.
In any case, ${\P^{B_0}_\lambda\subseteq\P^{B_0}_{\lambda,\emptyset}\cap\P^{B_0}_{\lambda,\kk}}$, so we are done if we can verify the hypotheses of \cref{P in B0C mod}.

We first check that for every $i=1,\ldots,n$, every entry in row $k_i=n-i+1$ is nonpositive.  
We observe that $B_i$ is obtained from $B_0$ for each $i=1,\ldots,n$, by negating the last~$i$ columns and then negating the last $i$ rows.
(Thus the entries of the bottom-right $i\times i$ submatrix are unchanged.)


OK, this doesn't work except for $i=1$

But it should work if instead we take the "backwards" piece of the bipartite Cox. el't for $B_0$ bipartite.
...
\rn{Drat.  Yes, is a problem.  Mutation need not preserve salience.  If this falls then \cref{finite P point} falls, and that's crucial to the proof of \cref{affine main}.  (Less importantly, also \cref{affine P point} falls.)}  
}
\end{proof}
...
Thus Assertion~\ref{eta lim} implies that for large enough or small enough $i$, the $\bigl(\eta_{12\cdots n}^{B_0^T}\bigr)^i(x)$ are in the same domain of linearity of $\eta^{B^T}_{12\cdots n}$ and $\eta^{B^T}_{12\cdots n}$agrees with $c$ on $\bigl(\eta_{12\cdots n}^{B_0^T}\bigr)^i(x)$.
...
Write $\lambda$ as $\lambda_0+\lambda_\infty$, where $\lambda_0$ is zero in the affine indices and $\lambda_\infty$ is a nonnegative scaling of $-\frac12B\delta^B$, so that ${\eta^{B^T}_\ll(\lambda)=\eta_\kk^{\Comp(B)^T}(\lambda_0)+\lambda_0}$ by \cref{factor eta}.
