...
\subsection{Criteria for the dominance region to be a point}\label{point sec}
We now use the tools developed so far to give sufficient conditions for the dominance region to be a point.
We use this criterion to show that the dominance region is always a point in finite type in both the coefficient-free case and when the extended exchange matrix has full rank (or satisfied a weaker condition that we describe below).
Later, we prove similar results for affine type.

Say an extended exchange matrix $\tB$ is \newword{salient} if the nonnegative linear span of the columns of $\tB$ contains no line.
(This term is a reference to convex geometry, where a closed cone is called salient if and only if it contains no line.)
The hypothesis that the $\tB_0$ is salient is strictly weaker than requiring that the columns of $\tB_0$ are linearly independent.
One can show the $\tB_0$ is salient by exhibiting a vector $x\in\reals^m$ whose dot product is strictly positive with every nonzero column of $\tB_0$.
...
We use \cref{P point} to prove a result about dominance regions in the coefficient-free finite-type case.
...
If $B$ is bipartite, then $B$ is salient.
...
Source-sink mutations on this tree give a bipartite exchange matrix $B_0$, so that $B_0$ is salient.
...
To prove these theorems, we quote some known results about the affine case and prove some new ones.
We begin with some tools to show that a given matrix is not of affine type and to determine exchange relations.
Then we quote and develop the relevant theory for different classes of affine-type seeds, namely acyclic seeds, arbitrary seeds, and neighboring seeds.

\subsection{Some tools}
We now gather some tools that will be useful in the rest of the section.
These tools include ways to show that certain exchange matrices are not of affine type by considering some small submatrices, as well as some details about exchange relations with principal coefficients.
...
We first construct a salient exchange matrix $B_0$ mutation equivalent to $B$.
...
When this graph is an oriented tree, source-sink moves make a bipartite (and therefore salient) exchange matrix $B_0$ mutation equivalent to $B$.
...
\end{align*}
We see that for any combination of the parities of $k$ and $n-k$, the inequalities can all be satisfied, and we conclude that $B_0$ is salient in every case.
