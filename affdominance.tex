...
If $\kk^{-1}=k_1\cdots k_m$ is a green sequence for $-B_t$, then 
\[\P_{\lambda,\kk}\subseteq\L_{\kk^{-1}}^{B_t^T}\set{\eta_\kk^{B_0^T}(\lambda)+B_t\alpha:\alpha\ge0}.\]
\end{theorem}

Moving towards the proof of \cref{eta in L}, we first determine the matrix for $\L_{\kk^{-1}}^{B_t^T}$ acting on column vectors.
...
Since $G_t^{B_0;t_0}$ is the matrix for~$\L_{\kk^{-1}}^{B_t^T}$ and since $\L_{\kk^{-1}}^{B_t^T}\eta_\kk^{B_0^T}(\lambda)=\lambda$, we rewrite the right side of the containment in \cref{eta in L} as follows.
...
\cref{B0C} immediately implies the following statement that is weaker than \cref{eta in L}.

\begin{proposition}\label{on domain}
$\P_{\lambda,\kk}\cap D=\set{\lambda+B_0C_t^{B_0;t_0}\alpha:\alpha\ge0}\cap D$.
\end{proposition}

We now prove our first main result.

\begin{proof}[Proof of \cref{eta in L}]
In light of \cref{B0C}, the theorem is equivalent to 
\[\P_{\lambda,\kk}\subseteq\set{\lambda+B_0C_t^{B_0;t_0}\alpha:\alpha\ge0}.\]
Now \cite[Proposition~1.4]{NZ} says that $C_t^{B_0;t_0}=F^{B_1}_{\ep,k_1}C_t^{B_1;t_1}$, where $\ep$ is the sign of the $k_1$-column of $C_{t_1}^{-B_t;t}$.  
(The hypothesis that $\kk^{-1}$ is a green sequence for $-B_t$ determines $\ep$, but we leave $\ep$ unspecified for now in order to highlight later where this hypothesis is relevant.)
...
Now, the hypothesis that $\kk^{-1}$ is a green sequence for $-B_t$ says that $\ep=+1$, so that $e_k$ is a nonnegative linear combination of columns of~$C_t^{B_1;t_0}$.
