...
For each $\ell\in\integers$, a point $x\in \P^B_{\lambda,\kk^\ell}$ is $\bigl(\eta_{\kk^\ell}^{B^T}\bigr)^{-1}\bigl(\eta_{\kk^\ell}^{B^T}(\lambda)+\mu_{\kk^\ell}(B)\alpha^{(\ell)}\bigr)$ for some $\alpha^{(\ell)}$ with nonnegative entries $a_1^{(\ell)},\ldots,a_n^{(\ell)}$.
...
The fact that $x\in\bigcap_{\ell\in\integers}\P^B_{\lambda,\kk^\ell}$ means that $x$ determines a lamination $X^{(\ell)}$, as described above, for each $\ell\in\integers$, and furthermore implies the following fact, which we refer to below as the \newword{main observation}:
...
\label{theti fig 1}
\end{figure}
The sum of the shear coordinates of $\Lambda$ and~$\Theta^{(0)}$ is $\pm(w_j+z_i^{(0)})$ in position~$t_i$, because the second remarkable property assures that $\thet_i=L_j$ makes this contribution and the third remarkable property says that no other curves of $\Lambda$ or $\Theta^{(0)}$ contribute to this coordinate.
