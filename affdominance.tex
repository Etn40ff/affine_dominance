...
Then $\P^{B_0}_\lambda=\set{\lambda}$.  \sayN{If $B_0$ is bipartitie and $\set{k_1,\ldots,k_\ell}$ is one of the parts, then we can use the $k_i$-entry of $\eta_{k_n\cdots k_1}^{B_0^T}(\lambda)$, i.e.\ same mutation map for all $i$.  We could write that into the \lcnamecref{bip singleton}, but this version might be necessary if we decide we can do $\tilde A$.}
\end{corollary}
\begin{proof}
Take $t_0\overset{k_1}{\edge}t_1\overset{k_2}{\edge}\,\cdots\,\overset{k_m}{\edge}t_n=t$ with $B_{t_0}=B_0$ and write $B_i$ for each~$B_{t_i}$.
...
For convenience, we will assume that $B_0$ is indexed so that $\kk=n\cdots 1$ so that $k_i=i$ for all~$1$.
...
The matrix $C_t^{B_i;t_i}$ is diagonal, with entries $1$ in the first $\ell$ diagonal entries and $-1$ in the other diagonal entries.  
