...
Given a vector $\lambda\in\reals^n$, define $\P^B_{\lambda,\kk}=\left(\eta_{\kk}^{B^T}\right)^{-1}\set{\eta_\kk^{B^T}(\lambda)+B_t\alpha:\alpha\ge0}$.
...
\[\P^{B_0}_{\lambda,\kk}\subseteq\set{\lambda+G_t^{B_0;t_0}B_t\alpha:\alpha\ge0}=\set{\lambda+B_0C_t^{B_0;t_0}\alpha:\alpha\ge0}.\]
\end{theorem}
...
\begin{align*}
\L_{\kk^{-1}}^{B_t^T}\set{\eta_\kk^{B_0^T}(\lambda)+B_t\alpha:\alpha\ge0}
&=\set{\lambda+G_t^{B_0;t_0}B_t\alpha:\alpha\ge0}\\
...
Let $\lambda'=\eta^{B_0^T}_\ll\!(\lambda)$.
\cref{shift} says $\eta^{B_0^T}_\ll\!(\P^{B_0}_\lambda)=\P^{B_{t'}}_{\lambda'}$.
Thus it is enough to prove that $\P^{B_{t'}}_{\lambda'}=\set{\lambda'}$.
Since $\eta_\ll^{B_0^T}\left(\Cone_t^{B_0;t_0}\right)=\Cone_t^{B_{t'};t'}$, we have reduced the proof to the case where there is a maximal red sequence for $B_t$ starting from $t$ and ending at $t_0$.
...
Then \cref{P in B0C} says that $\P^{B_0}_{\lambda,\kk}\subseteq\set{\lambda+B_0C_t^{B_0;t_0}\alpha:\alpha\ge0}$.
...
Since $\left(\reals_{\le0}\right)^n$ is a cone in the mutation fan $\F_{-B_t}$ (for example, combining \cite[Proposition~7.1]{universal}, \cite[Proposition~8.9]{universal}, and sign-coherence of $C$-vectors) and also $\Cone_{t_0}^{B_t^T;t}$ is a cone in $\F_{-B_t}$, we see that $\Cone_{t_0}^{B_t^T;t}=\left(\reals_{\le0}\right)^n$.
Thus, up to permuting columns, $C_{t_0}^{-B_t;t}$ is the negative of the identity matrix.
We see that $\P^{B_0}_{\lambda,\kk}\subseteq\set{\lambda-B_0\alpha:\alpha\ge0}$.

Since also $\P^{B_0}_{\lambda,\emptyset}\set{\lambda+B_0\alpha:\alpha\ge0}$, and since the columns of $B_0$ are linearly independent,   \margin{Need at least the hypothesis that span of $B_t$ does not contain a line! Here I have just taken "linearly independent columns".  We will need to do extended matrices in some form.  :(}
we conclude that $\P^{B_0}_\lambda=\set{\lambda}$.
\end{proof}
...
SS HOMEWORK:  Prove that the imaginary cone is in the domain of linearity when $t$ is a ``neighboring'' seed.

THEN:  We use the EBF trick again to rewrite $B_0C$ as $GB_t$. 
...
Given a vector $\lambda\in\reals^m$, define $\P^\tB_{\lambda,\kk}=\left(\eta_{\kk}^{\BB^T}\right)^{-1}\!\!\set{\eta_\kk^{\BB^T}(\lambda)+\tB_t\alpha:\alpha\in\reals^m,\alpha\ge0}$.
...
Here, we need to show that every vector $\left(E^{\BB_1}_{-\ep,k_1}\BB_1\CC_t^{\BB_1;t_1}\right)_{\col i}$ with $i\in\set{1,\ldots,n}$ whose $k_1$-entry has sign~$\ep$ is in $\posspan\set{\left(E^{\BB_1}_{\ep,k_1}\BB_1\CC_t^{\BB_1;t_1}\right)_{\col i}}_{i=1}^n$.
