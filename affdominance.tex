...
Then find a lamination whose shear coordinates, with respect to the rotated triangulation, are the sum of $-\mu_{\kk^\ell}(B)\alpha^{(\ell)}=-B\alpha^{(\ell)}$ and the shear coordinates of $\Lambda$.
...
Then take another lamination $\Theta^{(\ell)}$ whose shear coordinates are the negative of the vector $B\alpha^{(\ell)}$.
The lamination $\Theta^{(\ell)}$ has $n$ curves, namely, for $i=1,\ldots,n$, the curve $\thet_i$ with weight $z_i^{(\ell)}=a_{i-1}^{(\ell)}+a_{i+1}^{(\ell)}$, intersecting exactly one arc (the $i\th$ arc) in the rotated triangulation, as exemplified in the right picture of \cref{T fig}.  
%\begin{figure}
...
\label{theti fig 1}
\end{figure}
The sum of the shear coordinates of $\Lambda$ and~$\Theta^{(0)}$ is $\pm(w_j+z_i^{(0)})$ in position~$t_i$, because the second remarkable property assures that $\thet_i$ and $L_j$ make this contribution and the third remarkable property says that no other curves of $\Lambda$ or $\Theta^{(\ell)}$ contribute to this coordinate.
...
%\label{theti fig 2}
%\end{figure}
Since every curve in $\Lambda^{(\ell)}$ or $\Theta^{(\ell)}$, besides the rotation of $L_j$, is compatible with $\thet_m$, the third remarkable property says that the sum of the shear coordinates of $\Lambda^{(\ell)}$ and $\Theta^{(\ell)}$ is $\pm(w_j-z_m^{(\ell)})$ in position~$t_m$, with the sign determined by the orientation of $t_m$ in its quadrilateral.
But the main observation says that $X^{(\ell)}$ has the rotation of $L_j$ with weight $w_j+z_i^{(0)}$ or consists of curves that are compatible with the rotation of $L_j$.
Thus the sum of the shear coordinates of $\Lambda^{(\ell)}$ and $\Theta^{(\ell)}$ has the same sign and absolute value at least $w_j+z_i^{(0)}$ in position $t_m$.
We conclude that $z_i^{(0)}=z_m^{(\ell)}=0$.
