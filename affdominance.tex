...
Our proof takes stronger hypotheses than Qin's, but is sufficient for the purposes of the paper, uses only the basic discrete geometry of matrix mutation, and is short and self-contained.
...
Then $\P^{\tB_0}_\lambda=\set{\lambda}$ for all $\lambda\in\reals^n$.
\end{corollary}
\begin{proof}
...
\begin{corollary}\label{tree P point}  
Suppose $B$ is an exchange matrix of affine type.
If $t$ is a seed in the exchange graph for $\tB_0;t_0$ and $\lambda\in\Cone^{\tB_0;t_0}_t$, then $\P^{\tB_0}_\lambda=\set{\lambda}$.
\end{corollary}
\begin{proof}
...
Beyond rank $2$, the notion of affine type has the following intrinsic characterization.
...
(Indeed, the same can be said about cluster algebras of finite type.)
...
The matrix $B_0$ is skew-symmetrizable, meaning that there exist positive constants $d_1,\ldots,d_n$ such that the entries $b_{ij}$ of $B_0$ satisfy $d_i b_{ij}=-d_j b_{ji}$ for all $i,j$.
...
$\omega_c(x,c)=\omega_c(cx,cy)$ for any $x$ and $y$ in $V$.
\end{lemma}
...
Suppose $\RS$ is of affine type and suppose $\beta\in\RS$ is in a finite $c$-orbit.
Then $\omega_c(\beta,\delta)=0$.
\end{proposition}
\begin{proof}
Suppose the orbit of $\beta$ has $k$ elements.
The sum $\beta+c\beta+\cdots+c^{k-1}\beta$ of all roots in the $c$-orbit of $\beta$ is fixed by the action of $c$ and is nonzero, because all elements of the orbit have the same sign (positive or negative).
Thus the sum is an eigenvector of $c$, and thus a scalar multiple of $\delta$.
Because $\omega_c$ is skew-symmetric, $\omega_c(\beta+c\beta+\cdots+c^{k-1}\beta,\delta)=0$.
Now \cref{omega c} says that $k\omega_c(\beta,\delta)=0$.
\end{proof}
...
We conclude this section with a very brief discussion of $c$-sortable elements.  \sayN{We'll say more if we actually need a lot of details.}
...
This is a finite union of cones of codimension $1$, but it is not clear \textit{a priori} that $\d^B_\infty$ is a convex cone.
We will establish this fact below \sayN{I hope...}
and thus will again call $\d^B_\infty$ the \newword{imaginary wall}.
...
For this seed $t$ and green sequence $\kk$, define $\delta^B$ to be $\bigl(G_t^{-B_0^T;t_0}\bigr)^T\delta$.
...
\cref{delta is the man}.\ref{im hyp} establishes that $\d^B_\infty$ has codimension at most~$1$, and this exactly $1$, since it is a union of codimension-$1$ cones.
...
Up to relabeling, $B$ is of the form 
    $
      \begin{bsmallmatrix}
        B_{11} 	& 0 		& 0 		& B_{14} \\ 
        0 		& B_{22} 	& 0 		& B_{24} \\
      ...
      \end{bsmallmatrix}
    $ 
where $B_{44}=\begin{bsmallmatrix} 0 & a \\ -b & 0 \\\end{bsmallmatrix}$ with $a,b>0$ and $ab=4$,
satisfying the following conditions for $\ell\in\set{1,2,3}$:
\begin{itemize}
\item
...
\item
If $B_{4j\ell}$ is nonempty, then it is nonzero only in the last column; and
\item
If $B_{\ell4}$ is nonempty (equivalently if $B_{4\ell}$ is nonempty), then the nonzero rows and columns in $\begin{bsmallmatrix} 0 & B_{\ell4} \\ B_{4\ell} & B_{44} \end{bsmallmatrix}$ form one of the matrices in Table~\ref{tab:possible_submatrices}.
\end{itemize}
The labeling can be taken so that the blocks $B_{11}$, $B_{22}$, and $B_{33}$ are in order of increasing size. 
A submatrix in Table~\ref{tab:possible_submatrices} of types $A_{4}^{(2)}$, $G_{2}^{(1)}$, and $D_{4}^{(3)}$ can only appear when $B_{11}$ and $B_{22}$ are empty.
\end{enumerate}
\end{theorem}
	\begin{table}
...
Then every root $\gamma_i$ is $\beta_q+\beta_{q+1}+\cdots+\beta_r$ for some $1\le q\le r\le p_\ell$.
For the special index $i$, $\gamma_i$ is $\beta_1+\beta_2+\cdots+\beta_{p_\ell}$.
%\cref{compatible in tubes} says that roots

On the other hand, consider a $(p_\ell+3)$-gon with vertices labeled $0,1,\ldots,p_\ell+2$ and as initial triangulation, take all diagonals with an endpoint at $p_\ell+2$.
The roots $\gamma_i$ specify a triangulation that doesn't contain any of these initial arcs:
Each $\gamma_i=\beta_q+\beta_{q+1}+\cdots+\beta_r$ corresponds to the diagonal that crosses the initial diagonals with endpoints $q,q+1,\ldots,r$ and no other initial diagonals.  





Triangulations that don't contain any of these initial arcs correspond to 




The $\dd$-vectors with respect to that initial triangulation agree with the $\dd$-vectors $\gamma_i$, and the exchange relations agree.
We conclude that the non-special columns of $B_{\ell\ell}$ in this agree with this setup, which agrees with type A where one 
...
The point:  
The exchange relations, exchanging out anything but the highest root in the cluster, are exactly the exchange relations you would get in a 
  ...
  Moreover the (coefficient-free) exchange relations for $j< n-2$, $j\neq p_1,p_1+p_2$ are of the form
  \begin{equation}
    \label{eq:tube_rels}
    x_{\xi_j} x_\zeta = x_{\xi_j+\zeta} + x_{\xi_j\uplus\zeta}
  \end{equation}
If $\xi_j$ is not at the top layer, this is just affdenom facts...  
  
  for some $\zeta$ the $\g$-vector of a cluster variable, and $\xi_j\uplus\zeta$ (the symmetric difference of $\xi_j$ and $\zeta$\saySS{The standard notation for symmetric difference would have been $\Delta$ but it is definitely an overloaded symbol. I decided to follow CFZ's notation for the finite type case of which this is an instance I guess}) the $\g$-vector of a cluster monoimial, both in the same tube where $\xi_j$ lives.
  ...
  Indeed let $\ell$ be the minimum positive power such that $c^\ell$ fixes pointwise $\I$.
  Then the collection 
  \[
    \left\{x_{c^{j\ell} \xi_i}\right\}_{j\in\integers,i\in[1,n]}
  \]
...
ASSUMING that we have proved \cref{neigh B}, we continue.

\begin{proposition}\label{neigh delta}
Suppose $B$ is a neighboring exchange matrix.
Then $\delta^B$ is zero in all non-affine indices.
...
\end{proposition}
\begin{proof}
Considering a cluster pattern with $B$ as the initial exchange matrix, the initial $\g$-vectors for non-affine indices are contained in the imaginary wall.
...
What about the exact affine entries?
THIS IS TRICKY.  WHAT LEVERAGE DO WE HAVE?

But it's definitely what we need.

\end{proof}

As an immediate consequence of \cref{neigh delta}, we have the following proposition.

\begin{proposition}\label{neigh im ray}
Suppose $B$ is a neighboring exchange matrix.
...
% JUST KEEPING this old version, in case it is helpful...
%\begin{enumerate}[\bf1.]
%\item
...
\end{proposition}
\begin{proof}
By inspection of Table~\ref{tab:possible_submatrices} and in light of \cref{neigh delta,neigh im ray}, we observe the following facts:
...
Fourth, each non-affine column of $B$ equals the corresponding column of $\Comp(B)$ plus a nonnegative multiple of $-\frac12B\delta^B$. 
...
Furthermore, since $k$ is not special and not affine, column $k$ only has nonzero entries within one of the blocks $B_{\ell\ell}$ for $\ell\in\set{1,2,3}$.
Thus mutating $B$ at $k$ amounts to replacing $B_{\ell\ell}$ by $\mu_k(B_{\ell\ell})$.
The fact that $\Comp(\mu_k(B))=\mu_k(\Comp(B))$ now follows from the easy observation that mutating $B_{\ell\ell}$ at $k$ commutes with applying a positive scaling to a column of $B_{\ell\ell}$ other than~$k$.

\cref{neigh im ray} says that $\frac12B\delta^B$ is zero in all non-affine entries.
...
\end{proposition}
\begin{proof}
The special index $k$, together with the affine indices, determines a $3\times3$ submatrix of $B$ that agrees with one of the entries in Table~\ref{tab:possible_submatrices}.
...
0&\text{otherwise},
\end{cases}\]
and we will use the identity $m(a,b)+m(a,-b)=ab$ for $b>0$ several times.
...
\cref{neigh im ray} says that the vector $-\frac12B\delta^B$ that spans the imaginary ray is zero in non-affine entries and has affine entries $-1,1$.
...
The matrices of type $A_4^{(2)}$ shown in Table~\ref{tab:possible_submatrices} are obtained from the matrix of type $C_1{(1)}$ by conjugating by a diagonal matrix (with diagonal entries either $1,2,1$ or $1,1,2$).
...
The classification of affine root systems thus implies that $n=3$, so that $B$ in fact it equals one of the submatrices shown in Table~\ref{tab:possible_submatrices}.
...
The matrix of type $D_4^{(3)}$ in Table~\ref{tab:possible_submatrices} is obtained from the matrix of type $G_2{(1)}$ by conjugating by a diagonal matrix with diagonal entries $3,1,1$.
...
$\set{\eta_\ll^{B^T}(\lambda)+B_t\alpha:\alpha\in\reals^n,\alpha\ge0}\cap(\delta^B)^\perp\subseteq$


We will show that 


 $\P^B_{\lambda,\ll}=\bigl(\eta_{\ll}^{B^T}\bigr)^{-1}\set{\eta_\ll^{B^T}(\lambda)+B_t\alpha:\alpha\in\reals^n,\alpha\ge0}$




\cref{P point finite} says that the intersection of 
