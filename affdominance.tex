...
Let $\ll$ be a sequence of indices such that $\tB_1=\mu_\ll(\tB_0)$.
...
The Cartan matrix $A_0$ is the matrix of a bilinear form $K$ on $V$, in the simple-roots basis on the right and the simple-co-roots basis on the left.
...
The subspace $\set{v\in V:K(\gamma_c,v)=0}$ is the direct sum of all eigenspaces of $c$ so $c^m$ fixes that subspace pointwise.
...
Applying Assertion~\ref{c pos} and using the fact that $c\gamma_c=\gamma_c+\delta$, we have $\tau_c^m(\tau_c^i(\beta))=qm\delta+v$ and $\tau_c^{-m}(\tau_c^i(\beta))=-qm\delta+v$, and Assertions~\ref{exists} and~\ref{lim} follow.
\end{proof}
...
%%NO, this used the other piece of \cref{eta nice}.
%\item \label{c agree}
%$\eta_{12\cdots n}^{B^T}$ agrees with $c$ on $\bigl(\eta_{12\cdots n}^{B^T}\bigr)^i(x)$ for $i>I_+$ or $I<I_-$.
\item 
...
Thus $(\nu_c)^{-1}(x)$ is in a maximal cone of $\Fan_c(\RS)$ spanned by $\beta_1,\ldots,\beta_n$, indexed with $\beta_1,\ldots,\beta_\ell\in\AP{c}\setminus\APT{c}$ and $\beta_{\ell+1},\ldots,\beta_n\in\APT{c}$ with $\ell\ge1$.
...
Thus Assertion~\ref{pos a} implies that for large enough or small enough $i$, $\brr{\bigl(\eta_{12\cdots n}^{B^T}\bigr)^i(x),\beta}<0$ for all $\beta\in\RSfin$ such that $\omega_c(\beta,\delta)>0$.
This is Assertion~\ref{in big dinf}.  
%NOPE
%, and Assertion~\ref{c agree} follows by \cref{eta nice}.\ref{eta is c bigger}.
...
\cref{affine salient} allows us tochoose $B$ to be a salient exchange matrix in the exchange pattern.
