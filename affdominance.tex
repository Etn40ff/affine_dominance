...
The lamination $\Theta^{(\ell)}$ has $n$ curves, namely, for $i=1,\ldots,n$, the curve $\thet_i$ with weight $b_i^{(\ell)}=a_{i-1}^{(\ell)}+a_{i+1}^{(\ell)}$, intersecting exactly one arc (the $i\th$ arc) in the rotated triangulation, as exemplified in the right picture of \cref{T fig}.
%\begin{figure}
...
\label{theti fig 1}
\end{figure}
The sum of the shear coordinates of $\Lambda$ and $\Theta^{(0)}$ is $\pm(w_j+b_i^{(0)})$ in position~$t_i$, because the second remarkable property assures that $\thet_i$ and $L_j$ make this contribution and the third remarkable property says that no other curves of $\Lambda$ or $\Theta^{(\ell)}$ contribute to this coordinate.
Therefore, the second remarkable property says that $X^{(0)}$ has the curve $L_j=\thet_i$ with weight $w_j+b_i^{(0)}$ (or, if $w_j+b_i^{(0)}=0$, $X^{(0)}$ consists of curves that are compatible with $\thet_i$).
...
%\label{theti fig 2}
%\end{figure}
Since every curve in $\Lambda^{(\ell)}$ or $\Theta^{(\ell)}$, besides $\thet_m$ (which equals the rotation of $L'_j$), is compatible with $\thet_m$, the third remarkable property says that the sum of the shear coordinates of $\Lambda^{(\ell)}$ and $\Theta^{(\ell)}$ is $\pm(w_j-b_m^{(\ell)})$ in position~$t_m$, with the sign determined by the orientation of $t_m$ in its quadrilateral.
But the main observation says that $X^{(\ell)}$ has the rotation of $L_j$ with weight $w_j+b_i^{(0)}$ or consists of curves that are compatible with the rotation of $L_j$.
Thus the sum of the shear coordinates of $\Lambda^{(\ell)}$ and $\Theta^{(\ell)}$ has the same sign and absolute value at least $w_j+b_i^{(0)}$ in position $t_m$.
We conclude that $b_i^{(0)}=b_m^{(\ell)}=0$.

\smallskip

\noindent
\textit{Claim between cases (Type A).}
We claim that for each $j=1,\ldots,n$, the lamination $X^{(0)}$ has the curve $L_j$ with weight $\ge w_j$, or, if $w_j=0$ and $X^{(\ell)}$ does not have the curve $L_j$, then $X^{(\ell)}$ consists of curves that are compatible with $L_j$.
...
As in Case 1, we see that $X^{(\ell)}$ has the curve~$\thet_i$ with weight $w_j+b_i^{(\ell)}\ge w_j$, or, if not, then $w_j+b_i^{(\ell)}=0$ and $X^{(\ell)}$ consists of curves that are compatible with $\thet_i$.
...
\label{theti fig 3}
\end{figure}
The claim implies that each of the curves $L_m$ in $\Lambda$ that are incompatible with $\thet_i$ also appears in~$X^{(0)}$ with weight at least $w_m$ (or is compatible with every curve in $X^{(0)}$).
...
Thus the shear coordinate of $X^{(0)}$ at position $t_i$ is the sum of these weights $w_m$, and thus $b_i^{(\ell)}=0$.
...
For $i=4,\ldots,n$, the curve $\thet_i$ with weight $b_i^{(\ell)}=a_{i-1}^{(\ell)}+a_{i+1}^{(\ell)}$ and intersects~$t_i$ (taking $a_{n+1}^{(\ell)}=0$ as before).
%\begin{figure}
