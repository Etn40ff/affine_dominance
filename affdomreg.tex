...
The proofs draw on known results from the doubled Cambrian fan and almost-positive roots models, as well as a new tool that we develop: a detailed description of neighboring seeds of affine type (seeds that are, in some sense, as close as possible to the boundary of the $\g$-vector fan).
\end{abstract}
...
\sayS{Rephrased the following sentence slightly.  \rn{Changed "halfspace" to "half-hyperplane", because to me, "halfspace" carries the connotation of "full-dimensional".}}
Specifically, we show that with respect to a neighboring seed having exchange matrix $B$, the imaginary wall is a half-hyperplane and the restriction of the mutation fan to the imaginary wall is (\emph{metrically}, not merely \emph{combinatorially}) a product of the imaginary ray and some number ($1$, $2$ or $3$) of mutation fans of finite type~C.
...
\sayS{Small edits.  \okn}
A proof of this statement, which we had initially assumed would be trivial, turned out to be very difficult to find.
The one we give involves a type-by-type argument using the marked surfaces model, folding, and computations.
...
\sayD{[NR:  Commented out earlier parts of this marginal note to focus on what we're still talking about.]
%We have some inconsistencies on punctuation and capitalization when we use an enumerate/itemize.  I can make the changes throughout once we settle on a convention.
%\rn{
%Before we make any changes, I wanted to mention the convention that is currently in place for  enumerate environments:  Roman numerals in parentheses (or lower case letters if nested inside something already numbered) for conditions (either listed as part of a definition or declared to be equivalent) and bold arabic numerals for assertions (or bold lowercase letters if nested inside something numbered).  We can do something different if you want, but I'd like to distinguish these two kinds of lists.  Changing our minds should be fairly easy because we're (hopefully) referring to everything by reference.\\
...
%\rn{
%OK, I made some changes.  For this one:\\
%Suppose $\lambda'=\eta^{B^T}_\kk(\lambda)$ and $B'=\mu_\kk(B)$.
...
%}
\rd{I am happy now in most places but Prop 4.16 and Prop 4.17 feel like the numbering should be different since the sentence structure is different.  We should also compare with Props 4.52 and 4.53.}
\rn{I made a change to Proposition~\ref{eta nice} that brings it in line with what we do in this lemma.  (Old version commented out for easy reverting if we want.)  The change makes the beginning of each line crowded with notation, but maybe we can live with that.
I also made a change to Proposition~\ref{nonspecial mut} to bring it into line with the others.  (Again, old version commented out.)\\

If these changes are acceptable, then the remaining issue is Propositions~\ref{delta limit eta} and~\ref{special mut}.  These are the two numbered results where we say "there exists [something] such that [list of assertions]."  Currently, one has the assertions as a numbered list of complete sentences and the other has the assertions as a bulleted list and the assertions all fit together into a sentence that was started before the list.\\

Option A:  We change Proposition~\ref{delta limit eta} to bullet points to match Proposition~\ref{special mut}.  That would leave us with unnumbered assertions and we would have to fix how we refer to them.  We refer to them by number in two places outside of the proof.  Inside the proof, we refer to assertions by number a lot.\\

Option B:  We change Proposition~\ref{special mut} to numbered complete sentences.  The numbered complete sentences, in connection with a "there exists" are not independent "assertions" in the same sense as other numbered lists of assertions. \\

I think I prefer Option B.  
I don't think most readers will appreciate the subtle reason to typeset these differently with bullets, so it will just seem strange that they are different.
I can live with the subtle, "not independent assertions" still being numbered.  But I am willing to be persuaded.
}
}
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
\item \label{shift one}
...
Thus vectors in $\Cone^{B;t_0}_t$ all have weakly the same sign in the $k\th$ position.
...
Thus $C_t^{B_1;t_1}\bigl(C_t^{B_1;t_1}\bigr)^{-1}(\ep e_k)=\ep e_k$ is a nonnegative linear combination the columns of~$C_t^{B_1;t_1}$.
...
\noindent
\textbf{Type A.}
In the following, we assume $n\ge2$ to avoid separate explanations for type~$A_1$, which can instead be handled by direct inspection.
%To avoid corner cases we deal with type $A_1$ by direct inspection as for the exceptional types below and in the following assume $n\ge2$.
...
\sayS{Here we were claiming something false about the number of boundary arcs. If we want to be pedantic there is still the case of $n=1$. I do not know if it is better to add it here or to deal with it in the exceptional types.  \rn{I don't understand this comment.}\rs{When $n=1$ it is not true that the quadrilateral enclosing $t_1$ has only 3 boundary components. I added a comment about this at the beginning of the proof for this type.}  \rn{Oh, I see.  Good catch.  I rephrased the comment, because I wasn't sure what "corner cases" meant.}}
...
\sayS{The definition of $\APTre{c}$ that we had here was wrong.  \rn{It said ``Let $\APTre{c}$ be the set of positive real roots in $\RST{c}$ whose support is strictly smaller than $\Simples$.'' and I still think that's correct.  (We said ``smaller than $\Simples$'', so we meant $\Supp$, not $\SuppT$.)  I'm trying for a combination of the two ideas, because I think the ``support smaller than $\Simples$'' idea is simpler.  Is this OK?}   \rs{No it is not, here is the counterexample: In type $D_4^{(1)}$ let 3 be the central node and consider the usual Coxeter element $c=s_1\dots s_5$. Then there is a finite orbit consisting of $\alpha_3$ and $\sum_{i=1}^5\alpha_i$; the latter has full support. I am reverting your change.}  \rn{Oh,  see.  Yes, I'm okay with this as corrected by SS.}}
...
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
\item \label{nu tau}   
$\eta^{B^T}_{12\cdots n}\circ\nu_c=\nu_c\circ\tau_c$ as maps on $\AP{c}$.
\item\label{eta aut mut}
$\eta^{B_0^T}_{12\cdots n}$ is an automorphism of $\F_{B^T}$.
\item\label{eta is c}
$\eta^{B_0^T}_{12\cdots n}$ fixes $\d_\infty$ as a set, agrees with $c$ on $\d_\infty$, and has finite order on $\d_\infty$.
\end{enumerate}
\end{proposition}

%This is the version of the proposition before Nathan made some changes 26 Nov 2025
%\begin{proposition}\label{eta nice} 
%Suppose $B_0$ is an acyclic exchange matrix of affine type and the Coxeter element associated to $B_0$ is $c=s_1\cdots s_n$.
...
These $v$ and $t$ can be chosen so that $\bigl(G_{t_p}^{-B_0^T;t_0}\bigr)^T\delta$ has positive entries for all ${p=0,\ldots,r}$.
\end{proposition}
\begin{proof}
...
These are the cluster variables $x_{qr'}$, $x_{q'r}$, $x_{q(q'-2)}$, and $x_{(r+2)r'}$, except that the last three only exist if, respectively, $q'\le r$, $q\le q'-2$, and $r'\ge r+2$.
(In what follows, if $x_{q'r}$ does not exist, the notation $x_{q'r}$ will mean $1$, and similarly for $x_{q(q'-2)}$ and $x_{(r+2)r'}$.)
he dashed lines in the left picture of Figure~\ref{tube fig} indicate $\beta_{qr'}$, $\beta_{q'r}$, $\beta_{q(q'-2)}$, and $\beta_{(r+2)r'}$.
...
Thus Proposition~\ref{delta is the man}.\ref{im hyp} implies that $\delta^B$ is zero in all non-affine positions.
  (More carefully, in light of~Remark~\ref{danger: co-roots}, the simple-co-root coordinates of $\delta^B$ are zero in non-affine positions, and therefore the same is true of the simple-root coordinates that we take as the ``entries'' of $\delta^B$.) 
...
\item
The mutation map $\eta^{B^T}_k$ fixes the imaginary ray pointwise.
\item
If $x$ is a vector whose affine entries are zero, then $\eta^{B^T}_k(x)$ is again a vector whose affine entries are zero; writing $y$ for the projection of $x$ onto the subspace of non-affine indices, the non-affine entries of $\eta^{B^T}_k(x)$ coincide with the entries of the vector $\eta^{\Comp(B)^T}_k(y)$.
\end{enumerate}
\end{proposition}
%Version of the prop before NR made changes 26 Nov 2025
%\begin{proposition}\label{nonspecial mut}
%Suppose $B$ is a neighboring exchange matrix and $k$ is a non-special, non-affine index of~$B$.
...
Moreover the affine entries in column $k$ of $B$ are zero.
Thus if $x$ is a vector whose affine entries are zero and $y$ is its projection onto the subspace of non-affine indices, then the non-affine entries of $\eta^{B^T}_k(x)$ and $\eta^{\Comp(B)^T}_k(y)$ coincide while the affine entries of $\eta^{B^T}_k(x)$ are zero.
\sayD{This convention of identifying spaces should probably be explicitly stated above, I thought I remembered this being done but I didn't see it so far in this reading.\rs{Are you thinking of the proof of Proposition~\ref{neigh im wall}?}\rn{I'm not sure I understand this conversation.}\rs{Dylan is complaining about $x$ being an $n$ vector while $\Comp(B)$ has $n-2$ rows. I tried to address this here and in the statement above.}  \rn{OK, that makes sense.  In this sentence, I replaced "we have that" by "then".}}
\end{proof}
...
The output of Proposition~\ref{any B is sort}, for this large enough $i$, is a $c$-sortable element $v$ such that the sequence $\kk=k_r\cdots k_1$ that reverses the indices in the $c$-sorting word for $v$ has ${t_0\overset{k_1}{\edge}t_1\overset{k_2}{\edge}\,\cdots\,\overset{k_r}{\edge}t_r=t}$ such that $\bigl(G_{t_p}^{-B_0^T;t_0}\bigr)^T\delta$ has nonnegative entries for all ${p=0,\ldots,r}$, and analogously for $t'$.
...
\rs{A single mathcal letter is probably the best choice here, how about $\mathcal{Z}$ (implemented to see how it looks like).}\rd{I am fine with any of the options except $\mathbb{Z}\mathcal{P}$ since this looks too much like $\mathbb{ZP}$ or $\mathbb{QP}$ for a semifield $\mathbb{P}$ and we don't mean anything similar.}
\rn{I like the option with only mathcal Z.  Thanks, Salvatore, this is much better.}
}
Given an integer point $\tilde\lambda\in\integers^m$, define $\IP^\tB_{\tilde\lambda,\kk}=\bigl(\eta_{\kk}^{\BB^T}\bigr)^{-1}\sett{\eta_\kk^{\BB^T}(\tilde\lambda)+\mu_\kk(\tB)\alpha:\alpha\in\integers^n,\alpha\ge0}$ for all sequences~$\kk$.
...
\begin{theorem}\label{affine main integral}  
  Suppose $\tB$ is an $m\times n$ extended exchange matrix such that $B$ is of affine type.
