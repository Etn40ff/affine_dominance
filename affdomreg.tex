...
Suppose $\lambda'=\eta^{B^T}_\kk(\lambda)$ and $B'=\mu_\kk(B)$.
  \sayD{\color{orange}[NR:  Commented out earlier parts of this marginal note to focus on what we're still talking about.]
...
I can live with the subtle, "not independent assertions" still being numbered.  But I am willing to be persuaded.
  ...
  \rd{I implemented the change in Proposition~\ref{special mut}, feel free to delete this comment chain if it looks good.}
}
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
\item \label{shift one}
$\eta^{B^T}_\kk\!\!(\P^B_{\lambda,\ll})=\P^{B'}_{\lambda',\ll\kk^{-1}}$ for any $\ll$.
...
We see that the columns of $G_{t_p}^{-B_0^T;t_0}$ pair strictly positively with $\delta$.
\end{proof}
...
\end{lemma}
\sayS{Is this lemma really new? \rd{Theorem 7.5 in the GSV book is similar but not identical to this so maybe it's a new observation.  Here is their statement:\\ Theorem 7.5. Let every seed in a cluster algebra $\mathcal{A}\in\mathfrak{A}(B)$ be uniquely defined by its cluster. Then two clusters are adjacent in the exchange graph of $\mathcal{A}$ if and only if they have exactly $n-1$ common variables. In particular, Conjecture 3.46(ii) holds true for $\mathcal{A}$ satisfying one of the conditions (i) or (ii) of Theorem 7.4.\\  Here is their Theorem 7.4:\\ Theorem 7.4. Let a cluster algebra $\mathcal{A}\in\mathfrak{A}(B)$ satisfy one of the following two conditions:
(i) $\mathcal{A}$ is of geometric type;
(ii) $\mathcal{A}$ is arbitrary and B is nondegenerate.
Then every seed in $\mathcal{A}$ is uniquely defined by its cluster (in other words, Conjecture 3.46(i) holds true for $\mathcal{A}$).\\
We can mention these results if you think they are relevant.}}
\begin{proof}
...
\begin{proposition}\label{special mut}
Suppose $B$ is a neighboring exchange matrix and $k$ is a special index of~$B$.
Then there exists a sequence $\kk$ of indices in $\set{k,n-1,n}$ with the following properties.
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
\item
$\mu_\kk(B)$ is neighboring.
\item
...
$\CompPlus(\mu_\kk(B))$ does not equal $\mu_k(\CompPlus(B))$, but $\CompPlus(\mu_\kk(B))$ agrees with $\CompPlus(B)$ in the affine rows.
\item
The mutation map $\eta^{B^T}_\kk$ fixes the imaginary ray pointwise.
\item
If $x$ is a vector whose affine entries are zero, then $\eta^{B^T}_\kk(x)=\eta^{\Comp(B)^T}_k(x)$.
\end{enumerate}
\end{proposition}
...
\label{sec:integral}
We conclude by characterizing the integral dominance regions of affine type. 
\marginN{\color{orange}Please rescue me from this bad notation.\rs{Sorry I can't :(}  \rn{Dylan?  Please?} \rd{I don't have any good suggestions either.}\rn{OK, the options I can think of are:\\
