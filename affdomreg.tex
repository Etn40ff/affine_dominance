...
\sayS{@Nathan I changed ``imaginary wall'' to ``boundary of the $\g$-vector fan'' at the end of the abstract; is it OK?}
\sayS{I changed $\tilde B$ to $\widetilde B$  \rn{After some thought, I'm not wild about this change.  It makes higher lines in the prose (see, for example, the 7th line of the first paragraph) and it makes the tilde too far above the $B$.  But I'll go with whatever the group decides.}\rs{I prefer widetilde for readibility but I agree that it is too high and ruins lines. I tried looking for alternatives but could not locate a good one}}
...
\sayS{Rephrased the following sentence slightly}
Specifically, we show that with respect to a neighboring seed having exchange matrix $B$, the imaginary wall is a halfspace and the restriction of the mutation fan to the imaginary wall is (\emph{metrically}, not merely \emph{combinatorially}) a product of the imaginary ray and some number ($1$, $2$ or $3$) of mutation fans of finite type~C.
...
\sayD{The comment below about extending by zeros was also addressed here.  \rn{Do we want to address that by removing one, or just leave it?}\rs{I would leave it as is}}
...
\rs{A single mathcal letter is probably the best choice here, how about $\mathcal{Z}$ (implemented to see how it looks like).}}
