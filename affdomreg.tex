...
Then there exist integers $I_+$ and $I_-$ such that   \sayN{At the moment, we only use Assertions~\ref{correct side} and~\ref{eta lim} in this paper.  Assertion~\ref{pos a} is currently used to prove Assertion~\ref{eta lim}.  Possibly, we can delete the other two, I may want to keep them if they end up being needed in \{afftheta\}.  But I'll re-evaluate that soon when \{afftheta\} stabilizes.}
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
