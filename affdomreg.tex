...
\sayS{I changed $\tilde B$ to $\widetilde B$}
...
As in this paper we seldom use mutation maps without a transpose we could have opted to drop the superscript in our notation; we refrained from doing so since we will need to discuss and adapt to our needs parts of some proofs already written elswere with our current notation.
\sayS{\color{orange}Added this sentence to justify our three levels notation}
...
\sayN{\color{orange}I like this ``caseless'' way of saying things.  Thanks.\rs{Me too} \rd{Great!}}
...
Thus the diagonal matrix $\symmetrizer$ with diagonal entries $d_1,\ldots,d_n$, applied to the simple-root coordinates of a vector, gives the simple-co-root coordinates \sayD{\color{orange}It seems to me that we should use "coroot" everywhere and write this as "simple-coroot coordinates", or as "simple-co-root coordinates otherwise.  Alternatively, we could just write "simple root" in place of "simple-root" everywhere it appears.
\rn{I prefer "simple-root" when it's used as an adjective as in "simple-root coordinates" or "simple root" when it is a noun.  For co-roots, similarly. I like the dash in "co-root", and "simple-co-root coordinates" works for me.} \rd{I made this change throughout.}} 
of that vector.
...
Let $\APT{c}=\APTre{c}\cup\set\delta$. \sayD{The $c$ here is a subscript while it is a superscript in other places, does it make sense to make this uniform and always use subscripts?} 
...
\]
\sayS{\color{orange}The inequality here used to say $b_{kn}>-\frac32b_{kp}$ which is wrong, please check me. \rd{Looks good now.}}
...
0&\text{otherwise}.
\end{cases}\]
To determine the affine entries, we need only the $2\times2$ affine submatrix \sayD{\color{orange}Changed this from "submatrices"\rs{OK}} of $B$.
...
Let $\CompPlus(C)$ be the $n\times(n-2)$-matrix \sayD{\color{orange}Should this be $(n-2)\times n$?  \rn{No, I think $n$ rows and $n-2$ columns is correct.} \rd{Oh yeah, I'm dumb}} 
...
\begin{proof}
  ...
  \sayS{\color{orange}I think this sentence has to go: it is in the claim \rd{As the proposition was written it looked like an assumption but as it says here it is a consequence of Proposition~\ref{neigh im wall}.  Since we need the notation for the statement, I tried a fix.}}
