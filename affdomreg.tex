...
\sayS{I changed $\tilde B$ to $\widetilde B$  \okn}
...
From there, we can give the same characterization for $\tilde\lambda$ in the imaginary cone for arbitrary $\tB$ of affine type, with no condition on linear independence of columns.
...
\sayN{\color{orange}Say more?  Say differently?\rs{How about this?} \rn{Yes, that's a good thing to say.  I revised it a bit.  The main motivation for revising is that it's not really the integer lattice, it's $\lambda$ plus the integer lattice.  That seemed annoying to say directly, so I came up with this.  (Old version commented out.)  OK?} \rd{I like this version so I made this orange.}  \okn}
...
\sayS{Do we need a reference for mutation maps?  \rn{If we need one here, it's \cite{universal}, or more specifically \cite[Definition~4.1]{universal}.}} 
...
\sayS{Added this sentence to justify our three levels notation.  \rn{Good idea.  I revised slightly.} \rd{Revised a little more.}}
...
I would make the same change in Lemma~\ref{shift extended}.
For Lemma~\ref{eta nice}, I would say ``Then there exist integers $I_+$ and $I_-$ such that the following assertions hold.''
In Theorem~\ref{neigh B}\ref{neigh detailed}, we should delete the colon and replace all the semicolons with periods. 
}}
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
\item \label{shift one}
$\eta^{B^T}_\kk\!\!(\P^B_{\lambda,\ll})=\P^{B'}_{\lambda',\ll\kk^{-1}}$ for any $\ll$.
...
In other words, mutating the matrix $\begin{bsmallmatrix}B_0\\I\end{bsmallmatrix}$ by the sequence $\kk$ yields the matrix $\begin{bsmallmatrix}B_r\\-P\end{bsmallmatrix}$ for some permutation matrix $P$.
  \sayS{This proof was not correct as it was not properly taking care of the permutation, please check my fix.\rd{Looks good to me.}}
...
The base case, where $\kk=\emptyset$, is true because $C_{t_0}^{B_0;t_0}$ is the identity matrix and $\P_{\lambda,\emptyset}=\set{\lambda+B_0\alpha:\alpha\ge0}$.
  \sayD{I prefer $\varnothing$ to $\emptyset$, any complaints about switching?}
...
%Since $E^{B_1}_{\ep,k_1}$ is its own inverse, the claim is equivalent to saying that $E^{B_1}_{\ep,k_1}$ is the linear map that agrees with $\eta_{k_1}^{B_0^T}$ on $\Cone_t^{B_0;t_0}$.
  \sayD{\color{orange}Was this reformulation actually used in the next paragraph?  \rn{No, the next paragraph is the proof of the claim.  We use the reformulation later in the paragraph that starts "We have seen..."  We could move the reformulation later, I guess.  UPDATE:  I'm not sure we use the reformulation at all.  I have now commented it out and rephrased a bit.}\rd{Looks good now.}}
...
\[\eta_{\kk^{-1}}^{B_r^T}\set{\eta_\kk^{B_0^T}(\lambda)+B_r\alpha:\alpha\ge0}\subseteq\eta_{k_1}^{B_1^T}\set{\lambda'+B_1C_t^{B_1;t_1}\alpha:\alpha\ge0}.\]
  \sayS{\color{orange}Was $\eta_{\kk^{-1}}^{B_r^T}\set{\eta_\kk^{B_0^T}(\lambda')+B_r\alpha:\alpha\ge0}\subseteq\eta_{k_1}^{B_1^T}\set{\lambda'+B_1C_t^{B_1;t_1}\alpha:\alpha\ge0}$\rd{Looks good}}
...
\sayN{\color{orange}Commented out above is an a alternative to this paragraph that uses sign-coherence of $C_t^{B_1;t_1}$.  If this paragraph is wrong, maybe sign-coherence really was necessary after all.\rd{I didn't notice any issues.}}
...
Define $\BB$ to be the \sayS{\color{orange}Can we really use ``the'' here, $R'$ is not unique? \rd{I changed the previous sentence to eliminate any ambiquity.}\rs{OK}} matrix with block form $\begin{bsmallmatrix}B\,&-(R')^T\\R\,&0\end{bsmallmatrix}$.
...
If $\tilde\lambda'=\eta_\kk^{\BB^T}(\tilde\lambda)$ and $\tB'=\mu_\kk(\tB)$, then 
\begin{enumerate}[label=\bf\arabic*., ref=\arabic*]
...
We see that $\P^{\tB_0}_{\tilde\lambda,\kk}\subseteq\set{\tilde\lambda-\tB_0\alpha:\alpha\in\reals^n,\alpha\ge0}$.
  ...
  Since also $\P^{\tB_0}_{\tilde\lambda,\emptyset}=\set{\tilde\lambda+\tB_0\alpha:\alpha\in\reals^n,\alpha\ge0}$, and since $\tB_0$ is has linearly independent columns, we conclude that $\P^{\tB_0}_{\tilde\lambda}=\set{\tilde\lambda}$.
\end{proof}
...
\sayS{Added this sentence. Should we say geometric and/or linear algebraic instead of combinatorial? \rd{``elementary proof''?} \rs{I am not sure about elementary: we have a theorem, he has a lemma...} \rd{I just mean that the proof is not conceptual and works directly from the definitions.}} 
...
If a lamination has fewer than $n$ curves, we always extend it to a set of $n$ compatible curves by adjoining additional curves with weight~$0$.
  \sayD{The comment below about extending by zeros was also addressed here.}
...
\sayN{Modified following sentence.  It said ``This choice of $\Lambda$ ensures that we may compute $\eta^{B^T}_{\kk^\ell}(\lambda)$ by...''  But there is nothing special about $\Lambda$ that lets us compute mutation maps that way.  That's just how you compute mutation maps in the surfaces model.\rs{OK} \rd{I think that sentence was meant to be refering to the negation present in the choice of $\Lambda$, but it reads just fine without it.}}
...
Thus, since every exchange matrix of finite type is mutation equivalent to a bipartite exchange matrix of finite type, we may as well assume that $B$ is bipartite, and thus salient by Lemma~\ref{bip sal}.
  ...
  Suppose $\tB$ is $m\times n$ and $B$ is bipartite of finite type and let $\tilde\lambda\in\reals^m$ with $\lambda=\Proj_n(\tilde\lambda)$.
  ...
  But also $\P_{\tilde\lambda}^\tB\subseteq\P_{\tilde\lambda,\emptyset}^\tB=\set{\tilde\lambda+\tB\alpha:\alpha\ge0}$.
  Therefore $\P_{\tilde\lambda}^\tB\subseteq\set{\tilde\lambda+\tB\alpha:\alpha\ge0,B\alpha=0}$.
...
The latter case of the theorem is \cite[Theorem 1.2]{RSDom} when $bc=4$.  \marginN{\color{orange}Fill this reference in?\rs{done and added some words to the effect that we avoid this case our proofs here}}
...
\sayD{\color{orange}We implicitly use properties of $\delta$ in many places below, it might be worthwhile to collect those observations somewhere as a proposition.\rs{I am listing here properties that I notice while reading in case we decide to do so. $\delta$ has full support.}\rs{UPDATE: I think that apart from full support we are fine.}}
...
Let $\APT{c}=\APTre{c}\cup\set\delta$. \sayD{\color{orange}The $c$ here is a subscript while it is a superscript in other places, does it make sense to make this uniform and always use subscripts?  \rn{Possibly it makes sense.  But I shudder at altering the notation that we have used in the \{affdenom\} paper, which was so notation-heavy.  I vote for this notation to be ``grandfathered in''.} \rd{OK}} 
