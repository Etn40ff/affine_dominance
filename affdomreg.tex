...
\sayD{For symmetric $B$ this is fine but this requires a bit more care in general.  If $\symmetrizer B\symmetrizer^{-1}=-B^T$, then in order for $\BB$ to remain skew-symmetrizable we would need to have an $(m-n)\times (m-n)$ diagonal matrix $\symmetrizer'$ with $\symmetrizer'R=R\symmetrizer$ which may not exist.  If we were to allow rational entries we could replace $-R^T$ in $\BB$ with $-\symmetrizer^{-1}R^T$ and fix the issue.  An alternative would be to choose as the extension of $\symmetrizer$ any $\symmetrizer'$ so that $\symmetrizer'R\symmetrizer^{-1}$ has integer entries and then replace the $-R^T$ in $\BB$ by $-\symmetrizer^{-1}R^T\symmetrizer'$.  \rn{I liked your alternative, and implemented it.}} 
...
Then~$v$ and~$t$ can be chosen so that $\bigl(G_{t_p}^{-B_0^T;t_0}\bigr)^T\delta$ has nonnegative entries for all ${p=0,\ldots,m}$.
\end{proposition}
\begin{proof}
...
Since $C^{k_r}_c(v_{r-1})\ck$ is a positive scalar multiple of $C^{k_r}_c(v_{r-1})$, we need to check that $\omega_c(C^{k_r}_c(v_{r-1}),\delta)\ge0$. 
...
If the $k_r$ entry of $-\frac12B_{m-1}\delta^B_{r-1}$ is strictly negative, then the $k_r$ entry of $\lambda$ is nonpositive, and thus $\eta^{B^T_{r-1}}_{k_r}$ acts on $\lambda$ (just as it acts on $-\frac12B_{r-1}\delta^B_{r-1}$) by the map given in the basis of fundamental weights by the matrix $E_{-,k_r}^{B_{r-1}}$.
