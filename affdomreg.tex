...
\marginN{Small changes in wording in the abstract}
...
\marginN{rewrote from here to end of paragraph.  Also fixed typo "collections" above.}
Dominance regions/orders are the crucial discrete-geometric construction that characterizes the class of pointed bases, a class that includes the theta basis~\cite{GHKK}, the generic basis~\cite{Dup11}, and the common triangular basis~\cite{QinTri}. 
...
\marginN{Rewrote the following paragraph to avoid diluting the impact with a discussion of ``rank''.  It doesn't seem like that discussion needs to be re-inserted anywhere else.  Also, grabbed part of the discussion on motivations from the end of the intro, to make it more prominent.}
...
The results of this paper are motivated by two applications.  
...
The other application combines the results of this paper with a result of \cite{canonical} that uses the dominance region to greatly simplify the computation of some structure constants for multiplication of theta functions in the cluster scattering diagram (in the sense of~\cite{GHKK}).
...
\marginN{Expanded parenthetical to mention Felikson-Tumarkin.  Also rephrased to make it clear that they only do *part* of the result.}
...
\marginN{This is where I grabbed some motivation from here to place it earlier.}
...
has been pointed out elsewhere, namely by Felikson and Tumarkin \cite{FeTuCoeff} in the context of a classification of extended exchange matrices of finite mutation type and by Greenberg and Kaufman \cite{GreenbergKaufman} in the context of a uniform description of the cluster modular groups of affine and extended affine types.  \marginN{Modified this to mention Felikson and Tumarkin as well.}
\end{remark}
