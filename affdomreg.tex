...
Therefore, $X^{(\ell_m)}$ has the curve $\theta_{i_m}$ with weight $w_m+z_{i_m}^{(\ell_m)}$.
...
Since every curve in $\Lambda^{(\ell'_m)}$ or $\Theta^{(\ell'_m)}$, besides the rotation of $L_m$, is compatible with $\theta_{i'_m}$, the third remarkable property says that the shear coordinate of $X^{(\ell'_m)}$ in position~$t_{i'_m}$ is ${\ep_{i'_m}(-w_m+z_{i'_m}^{(\ell'_m)})}$.
...
%\noindent
%\textit{Case 1 (Type A).}  
%The curve $\theta_i$ coincides with a curve $L_j\in\Lambda$.
%This case is represented in the left picture of Figure~\ref{theti fig 1}.
...
%\put(7.5,-20){$t_{\!m}$}
%\end{picture}}
%\caption{$\theta_i$ coincides with some $L_j$}
%\label{theti fig 1}
%\end{figure}
%The sum of the shear coordinates of $\Lambda$ and~$\Theta^{(0)}$ is $\ep_0(w_j+z_i^{(0)})$ in position~$t_i$ for $\ep_0\in\{\pm1\}$, because the second remarkable property assures that $\theta_i=L_j$ makes this contribution and the third remarkable property says that no other curves of $\Lambda$ or $\Theta^{(0)}$ contribute to this coordinate.
%Therefore, $X^{(0)}$ has the curve $L_j=\theta_i$ with weight $w_j+z_i^{(0)}$.
%
...
%By the first remarkable property, there exists $\ell\in\integers$ such that rotating $L'_j$ by $\ell$ units gives a curve~$\theta_m$, as exemplified in the right picture of Figure~\ref{theti fig 1}.
%%\begin{figure}
...
%The first remarkable property says that there exists $\ell\in\integers$ such that the rotation of $L_j$ counterclockwise $\ell$ units is a curve $\theta_i$.
%As in Case 1, we see that either $X^{(\ell)}$ has the curve~$\theta_i$ with weight $w_j+z_i^{(\ell)}\ge w_j$ or $w_j+z_i^{(\ell)}=0$ and $X^{(\ell)}$ consists of curves that are compatible with $\theta_i$.
...
%\noindent
%\textit{Case 2 (Type A).}
%The curve $\theta_i$ does not coincide with any curve $L_j\in\Lambda$.
%Since $\Lambda$ is maximal, there are curves in $\Lambda$ that are not compatible with $\theta_i$.
%The situation is pictured in Figure~\ref{theti fig 3}.
%\begin{figure}
...
%\put(15,-30){$t_i$}
%\end{picture}
%\caption{Curves in $\Lambda$ are incompatible with $\theta_i$}
%\label{theti fig 3}
%\end{figure}
%The claim implies that each of the curves $L_m$ in $\Lambda$ that are incompatible with $\theta_i$ also appears in~$X^{(0)}$ with weight at least $w_m$ (or $L_m$ is compatible with every curve in $X^{(0)}$).
%The third remarkable property says that each of these curves $L_m$ has shear coordinate strictly disagreeing in sign at position~$t_i$ with the shear coordinate of $\theta_i$ and that no other curves besides $\theta_i$ and these $L_m$ contribute to position $t_i$ of the sum of the shear coordinates of $\Lambda$ and $\Theta^{(0)}$.
...
Then there exists a $c$-sortable element~$v$ whose associated seed $t$ has exchange matrix $B$.  \sayN{Have we explained ``associated seed''?  (I guess, sort of, above, but only for finite type.)  I think we mean the seed at the end of the green sequecnce in Proposition~\ref{sort green}, but we should say so.}
...
We can represent each $\gamma_i$ pictorially as in Figure~\ref{tube fig},  \sayN{Need picture.  I can probably modify a PS file from \{affdenom\}.}
...
%Now, appealing to Lemma~\ref{neigh neg and T}, we know that $B^T$ is also neighboring.
%In particular, the argument to this point implies that the entries of $B_{\ell\ell}^T$ in non-special columns are $0$, $1$, or $-1$.
...
The diagonal associated to $\gamma_i$ is contained in a quadrilateral in~$T$, as illustrated in Figure~\ref{tri fig}.  \sayN{Need picture.  I can easily supply PS.}
...
\begin{proof}
NEED THIS.  \sayN{If we don't get this, we will know that the dominance region is a line segment, but will not know what the line segment is, except in a neighboring seed!}
(Note:  This is for arbitrary $\kk$, not just copies of $c$.)
