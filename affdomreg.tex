...
Thus if $x$ is a vector whose affine entries are zero (so that $\eta^{\Comp(B)^T}_k(x)$ makes sense\sayD{This convention of identifying spaces should probably be explicitly stated above, I thought I remembered this being done but I didn't see it so far in this reading.\rs{Are you thinking of the proof of Proposition~\ref{neigh im wall}?}}), we have $\eta^{B^T}_k(x)=\eta^{\Comp(B)^T}_k(x)$.
\end{proof}
...
\begin{proof}
  ...
  \sayS{\color{orange}I think this sentence has to go: it is in the claim \rd{As the proposition was written it looked like an assumption but as it says here it is a consequence of Proposition~\ref{neigh im wall}.  Since we need the notation for the statement, I tried a fix.}  \rn{I'm confused what this comment is talking about.} \rd{I think edits were made that resolved the issue so I'm making it orange.}}
...
The set $\P^B_{\lambda,\widehat\kk}$ is obtained by applying $(\eta_{\widehat\kk}^{B^T})^{-1}$ to $\set{\eta_{\widehat\kk}^{B^T}(\lambda)+B'\alpha:\alpha\in\reals^n,\alpha\ge0}$.
  By Proposition~\ref{factor eta}, if some point $x$ is in $\P^B_{\lambda,\widehat\kk}$  \sayN{Do we want $\P^B_{\lambda,\widehat\kk}\cap\d_\infty^{\mu_\ell(B)}$ instead? \rd{I think needing that would be bad because we want to know that the dominance region is already contained in the imaginary cone and not just make a statement about how they intersect.  Ah, Lemma 4.23 saves us and we can omit the intersection.}}
...
\label{sec:integral}
We conclude by characterizing the integral dominance regions of affine type.  \marginN{Please rescue me from this bad notation.\rs{Sorry I can't :(}  \rn{Dylan?  Please?} \rd{I don't have any good suggestions either.}}
Given an integer point $\tilde\lambda\in\integers^m$, define $\IP^\tB_{\tilde\lambda,\kk}=\bigl(\eta_{\kk}^{\tB^T}\bigr)^{-1}\sett{\eta_\kk^{\tB^T}(\tilde\lambda)+\mu_\kk(\tB)\alpha:\alpha\in\integers^n,\alpha\ge0}$ \sayS{Was $\IP^\tB_{\tilde\lambda,\kk}=\bigl(\eta_{\kk}^{\BB^T}\bigr)^{-1}\sett{\eta_\kk^{\BB^T}(\tilde\lambda)+\mu_\kk(\tB)\alpha:\alpha\in\integers^n,\alpha\ge0}$} for all sequences~$\kk$.
