...
\sayS{Yes, done it but only in the abstract since in the introduction we quote Qin in a general way}
...
(
\sayS{In~\cite{RSDom} we were agnostic about about this, hopeffully}
In this paper, we consider \emph{tall} extended exchange matrices rather than \emph{wide} ones.
Also, we follow the usual practice of taking the ``rank'' of a cluster algebra to be the number of columns in its extended exchange matrix.
...
We prove that when $\tB$ has linearly independent columns, the dominance region $\P_\lambda^\tB$ is a point if $\lambda$ projects to the $\g$-vector fan or otherwise (i.e.\ when $\lambda$ projects to the relative interior of the imaginary wall) is a line segment that we describe explicitly.
\sayS{Do we need to reiterate in the above sentence that we are talking about the affine case?}

\sayS{I rephrased the following paragraph and commented out the old version}
Computing dominance regions without the assumption that $\tB$ has linearly independent columns is in general is much harder. 
However, in affine type, our argument to deal with points that project to the relative interior of the imaginary wall passes through the coefficient-free case where we can describe the line-segments explicitly:
...
One project is to determine all pointed bases of cluster algebras of affine type, using the result of Fan Qin mentioned above \cite[Theorem~1.2.1]{FanQin} and relate them to certain generalized minors in double Bruhat cells as in \cite{RSW19}.
...
That is, $\mu_\kk(B)$ means $\mu_{k_r}(\mu_{k_{r-1}}(\cdots(\mu_{k_1}(B))\cdots))$.
...
The sequence $\kk$ is a \newword{green sequence} for an exchange matrix $B$ if column $k_p$ of $C_{t_{p-1}}^{B;t_0}$ is \emph{positive} for all $p$ with $1\le p\le r$.
...
\end{lemma}
\begin{proof}
By the definition of a red sequence, the lemma is equivalent to the statement that  $\kk=k_r\cdots k_1$ of indices is a maximal green sequence for $B_{t_0}$ if and only if $\kk^{-1}=k_1\cdots k_r$ is a maximal green sequence for~$-B_t$.
...
Also, given $\BB$, a sequence $\kk=k_r\cdots k_1$ of indices in $\set{1,\ldots,n}$, and seeds $t_1,\ldots,t_r$ by $t_0\overset{k_1}{\edge}t_1\overset{k_2}{\edge}\,\cdots\,\overset{k_r}{\edge}t_r=t$, there are associated matrices of $\g$-vectors and $\c$-vectors, which we write as $\GG_t^{\BB;t_0}$ and $\CC_t^{\BB;t_0}$.
...
\begin{proposition}\label{GBBC ext}  \sayN{Maybe we don't need this?}\sayS{It looks like we do not}
...
Any curve $\gamma\neq\theta_i$ that can appear in a lamination \sayN{Removed ``with $\theta_i$'' here.  It makes the remarkable property too weak to be useful.  The property is true for any curve that can appear in any lamination, and that's what makes the property powerful.\rs{OK} }
...
\sayN{Modified following sentence.  It said ``This choice of $\Lambda$ ensures that we may compute $\eta^{B^T}_{\kk^\ell}(\lambda)$ by...''  But there is nothing special about $\Lambda$ that lets us compute mutation maps that way.  That's just how you compute mutation maps in the surfaces model.\rs{OK}}
...
\marginN{Added the following.  We need it because later we're going to ask for $L'_p$ to be the unique curve $\neq L_p$ that is compatible with the others.  Without extending, we don't get uniqueness.\rs{OK}}
...
\sayN{Removed ``Observe that'' because we already said it in the previous paragraph, and it's not an observation, it's a result that we have to quote.'' \rs{OK}}
...
Since there is a unique lamination associated to each vector, we obtain the following fact, which we refer to below as the \newword{main observation}:  \sayN{I wanted to make the following sentence more precise by saying what the rotation is.  So I added the ``specifically'' sentence.\rs{OK}}
...
\sayN{Added the following. $X^{(\ell_p)}$ might *not* have $\theta_{i_p}$, and we need to be clear about what happens in that case.\rs{OK}}
...
It follows that $X^{(0)}$ contains the same curves as $\Lambda$ \sayN{Also added the following for the same reason.\rs{OK} }
...
\sayN{I like this ``caseless'' way of saying things.  Thanks.\rs{Me too}}
...
The latter case of the theorem is \cite[Theorem 1.2]{RSDom} in the special case where $bc=4$.  \marginN{Fill this reference in?\rs{done and added some words to the effect that we avoid this case our proofs here}}
...
\begin{remark}\label{danger: co-roots}  \sayN{Added this to ward off confusion, especially in Proposition~\ref{delta is the man}.\rs{OK}}
...
The subspace $\set{v\in V:K(\gamma_c,v)=0}$ is the direct sum of all eigenspaces of $c$, so $c^\ell$ fixes this subspace pointwise.
...
By Proposition~\ref{delta limit}, there exist integers $I_1,\ldots,I_\ell$ and positive real $a_1,\ldots,a_\ell$ such that $\tau_c^{i+\ell}\beta_j=(\tau_c^i\beta_j)+a_j\delta$ whenever $i>I_j$ and $\tau_c^{i-\ell}\beta_j=(\tau_c^i\beta_j)+a_j\delta$ whenever $i<I_j$, for all $j=1,\ldots,\ell$.
Set $a=\sum_{j=1}^\ell q_ja_j>0$.
Then $\tau_c^{i+\ell}((\nu_c)^{-1}(x))=\tau_c^i((\nu_c)^{-1}(x))+a\delta$ for $i>\max(I_1,\ldots,I_\ell)$ and $\tau_c^{i-\ell}((\nu_c)^{-1}(x))=\tau_c^i((\nu_c)^{-1}(x))+a\delta$ for $i<\min(I_1,\ldots,I_\ell)$.
Applying $\nu_c$ and appealing to Proposition~\ref{eta nice}.\ref{nu tau}, we see that $\bigl(\eta_{12\cdots n}^{B^T}\bigr)^{i+\ell}(x)=\bigl(\eta_{12\cdots n}^{B^T}\bigr)^i(x)+a\nu_c(\delta)$ for $i>\max(I_1,\ldots,I_\ell)$ and $\bigl(\eta_{12\cdots n}^{B^T}\bigr)^{i-\ell}(x)=\bigl(\eta_{12\cdots n}^{B^T}\bigr)^i(x)+a\nu_c(\delta)$ for $i>\min(I_1,\ldots,I_\ell)$.
...
Let $p$ be the order of $\eta^{B_0^T}_{12\cdots n}$ on $\d_\infty$ (see Proposition~\ref{eta nice}.\ref{eta is c}).
Then $\bigl(\eta^{B_0^T}_{12\cdots n}\bigr)^p$ fixes $\d_\infty$ pointwise.
Also, $\mu_{12\cdots n}(B_0)=B_0$, so Lemma~\ref{shift}.\ref{shift all} says ${\bigl(\eta^{B_0^T}_{12\cdots n}\bigr)^p(\P_\lambda^{B_0})=P_\lambda^{B_0}}$.
...
\item \label{im ray pos}
  $\delta^B$ has nonnegative entries and is the shortest integer vector in the ray it~spans.  \sayN{Added the statement that it's the shortest integer vector. Check me.\rs{It seems OK}}
\item \label{im ray}
...
\begin{proof}%[Proof of Proposition~\ref{delta is the man}]
  Assertion~\ref{im ray pos} is true because we chose $v$ and $t$ so that $\bigl(G_t^{-B_0^T;t_0}\bigr)^T\delta$ has nonnegative entries. \sayN{Rest of this paragraph is what I added to prove the added statement.\rs{OK}}
...
\sayS{This uses $c=s_1\dots s_n$ but we havent made this assumption in this section.}
...
\sayN{Salvatore, you said you were confused about the proof of the following proposition, and you were right to be!  It was wrong, and in fact the statement was wrong.  I think this is now correct and everything still works, but it's slightly more complicated \rs{Great!}}
...
\end{proposition}
\begin{proof}
  Proposition~\ref{neigh im wall} implies that we can write $\lambda$ as $\lambda_0+\lambda_\infty$.\sayD{Should one of these be a $\lambda_\infty$?  \rn{Yes, fixed.}}\sayS{I think this sentence has to go: it is in the claim}
