...
Let $\symmetrizer'$ be the diagonal matrix with minimal entries such that $R':=\symmetrizer'R\symmetrizer^{-1}$ has integer entries.
Define $\BB$ to be the \sayS{Can we really use ``the'' here, $R'$ is not unique? \rd{I changed the previous sentence to eliminate any ambiquity.}} matrix with block form $\begin{bsmallmatrix}B\,&-(R')^T\\R\,&0\end{bsmallmatrix}$.
Most importantly, $\BB$ is skew-symmetrizable by the diagonal matrix $\begin{bsmallmatrix}\symmetrizer&0\\0&\symmetrizer'\end{bsmallmatrix}$ and agrees with $\tB$ in columns $1$ to $n$.
  Throughout, if we have defined an extended exchange matrix $\tB$, without comment we will take $B$ to be the underlying exchange matrix and $\BB$ to be the\sayS{Ditto \rd{ditto}} associated $m\times m$ matrix.
...
\sayS{Added this sentence. Should we say geometric and/or linear algebraic instead of combinatorial? \rd{``elementary proof''?}}
...
\begin{proposition}\label{finite P point bip}
  ...
  Suppose $B$ is a bipartite $n\times n$ exchange matrix of finite type.
...
If $B$ is an exchange matrix of affine type, then $B$ admits a maximal green sequence and a maximal red sequence.
  \sayD{This argument is remarkably similar to the proof of Proposition 2.28 so I thought it could make sense to combine them, but it isn't clear if there is any way for this to actually work with the structure of the paper.}  
\end{proposition}
\begin{proof}  
...
0&\text{otherwise}.
\end{cases}\]
  To determine the affine entries, we need only the $2\times2$ affine submatrix \sayD{Changed this from "submatrices"} of $B$.
...
Thus $\xi_k$ is replaced by another vector in $\d_\infty$.
  \sayD{The notation of $\gamma$ and $\xi$ was introduced around 7 pages back, it seems like we should recall their meaning before using them again.}
...
Since $k$ is non-special, the non-affine entries of column $k$ of $B$ agree with column $k$ of $\Comp(B)$.
  Thus if $x$ is a vector whose affine entries are zero (so that $\eta^{\Comp(B)^T}_k(x)$ makes sense\sayD{This convention of identifying spaces should probably be explicitly stated above, I thought I remembered this being done but I didn't see it so far in this reading.}), we have $\eta^{B^T}_k(x)=\eta^{\Comp(B)^T}_k(x)$.
\end{proof}
...
\begin{proof}
  ...
  \sayS{I think this sentence has to go: it is in the claim}\sayD{As the proposition was written it looked like an assumption but as it says here it is a consequence of Proposition~\ref{neigh im wall}.  Since we need the notation for the statement, I tried a fix.}
