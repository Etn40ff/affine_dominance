...
\]
\sayS{The inequality here used to say $b_{kn}>-\frac32b_{kp}$ which is wrong, please check me. \rd{Looks good now.}}
...
Write $\alpha$ as $\beta+\gamma$ such that $\beta$ is a linear combination of simple roots in non-affine entries and $\gamma$ is a combination of simple roots in affine positions.
  \sayS{Should we avoid talking of roots when the underlying root system is not affine i.e. when the initial matrix is not acyclic? \rd{The root language doesn't seem necessary here.  We can just say ``$\beta$ is zero in its affine entries and $\gamma$ is zero in its non-affine entries.''}}
Then $B\beta$ is a nonnegative linear combination of the non-affine columns of $B$, and equivalently a nonnegative linear combination of the columns of $\CompPlus(B)$.
  Also $B\beta$ is in $(\delta^B)^\perp$ because every non-affine column of $B$ is.\sayD{This was observed somewhere earlier, maybe as a part of a proof, should we highlight it as a Lemma so that we can refer to it here?}
...
Thus the $\g$-vector $\xi_k$ is replaced by another vector in $\d_\infty$.
  \sayD{\color{orange}The notation of $\gamma$ and $\xi$ was introduced around 7 pages back, it seems like we should recall their meaning before using them again.\rs{added "$\g$-vector" to help with this} \okd}
