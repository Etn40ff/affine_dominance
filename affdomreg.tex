...
%Changes made after submission to Selecta are marked
%SinceSelecta
...
\address[N. Reading]{Department of Mathematics, North Carolina State University, %Raleigh, NC, 
USA}
...
Motivations for this work include a project to determine all pointed bases for cluster algebras of affine type and a separate application that determines all theta functions in the affine case.
...
\subjclass[2020]{Primary 13F60}
\keywords{Cluster algebras, dominance order, pointed bases}
\maketitle
...
Given an $m\times n$ extended exchange matrix $\tB$ and a point $\tilde\lambda\in\reals^m$, the dominance region $\P_{\tilde\lambda}^\tB$ is the intersection of an infinite collection of piecewise polyhedral sets, each defined by applying mutation maps to a (translated) cone.
...
Dominance regions/orders are the crucial discrete-geometric construction that characterizes the class of pointed bases of the (upper) cluster algebra associated to $\tB$, a class that includes the theta basis~\cite{GHKK}, the generic basis~\cite{Dup11}, and the common triangular basis~\cite{QinTri}. 
(More specifically, Fan Qin characterized all pointed bases of the upper cluster algebra in terms of this order, assuming that $\tB$ has linearly independent columns and admits a green-to-red sequence \cite[Theorem~1.2.1]{FanQin}.)
...
Rupel and Stella~\cite{RSDom} computed dominance regions in the case where $n=m=2$. 
They found that, in the $n=m=2$ affine case, dominance regions are either points or line segments.
In this paper, we generalize that fact to all extended exchange matrices of affine type for arbitrary $m\ge n$.
Our results are motivated by two applications.  
One is an ongoing project to determine all pointed bases of cluster algebras of affine type using \cite[Theorem~1.2.1]{FanQin} and to relate them to certain generalized minors in double Bruhat cells as in \cite{RSW19}.
The other application combines the results of this paper with a result of \cite{canonical} and uses dominance regions to greatly simplify the computation of some structure constants for multiplication of theta functions.
This allows us, in~\cite{afftheta}, to determine all theta functions for cluster scattering diagrams of affine type, determine ``imaginary'' exchange relations among them, and identify an ``imaginary subalgebra'' that is essentially a product of generalized cluster algebras of finite type $C$.
...
(One aspect of this study appears in work of Felikson and Tumarkin~\cite{FeTuCoeff} and of Greenberg and Kaufman~\cite{GreenbergKaufman}.
See Remark~\ref{related remark}.)
...
For our applications, the relevant notion is the integral dominance region: the set of points in $\P_{\tilde\lambda}^\tB$ that are obtained as $\tilde\lambda$ plus an \emph{integer}-linear combination of columns of $\tB$.
...
For $k\in\set{1,\ldots,n}$ and either choice of $\ep\in\set{\pm1}$, the mutation of~$\tB$ at $k$ is $\mu_k(\tB)=E_{\ep,k}^\BB \tB F_{\ep,k}^B$.
\end{lemma}
...
%SinceSelecta:  Corrected "restriction" citation to Theorem 3.4 and added reference to 2.9
The fan $\nu_c(\Fan_c(\RS))$ is the mutation fan for $B_0^T$ \cite[Theorem~2.9]{affscat}, and the map $\nu_c$ restricts to an isomorphism from the $\dd$-vector fan of $B_0$ to the $\g$-vector fan \cite[Theorem~3.4]{affscat}. 
...
%SinceSelecta:  Corrected 7.31 to 7.33
The following proposition is part of \cite[Proposition~7.33]{affscat}.
...
%SinceSelecta:  Corrected 7.31 to 7.33
The proof of Proposition~\ref{eta nice}.\ref{eta is c} in \cite[Proposition~7.33]{affscat} includes checking, for each of the $n$ mutation steps of applying $\eta^{B^T}_{12\cdots n}$ to $\nu_c(\delta)$, which of the cases of the mutation map applies.
...
\begin{remark}\label{related remark}
The existence, in each mutation class of affine type, of a matrix satisfying Condition~\ref{neigh detailed} 
has been pointed out elsewhere, namely by Felikson and Tumarkin \cite{FeTuCoeff} in the context of a classification of extended exchange matrices of finite mutation type and by Greenberg and Kaufman \cite{GreenbergKaufman} in the context of a uniform description of the cluster modular groups of affine and extended affine types.  
\end{remark}
...
%SinceSelecta:  Defined the projection that we should be using multiple times.
%ACTUALLY, didn't do this!
%If $x$ is a vector whose affine entries are zero, write $x_\na$ for the projection of $x$ onto $\reals^{n-2}$ that deletes the affine indices.
%SinceSelecta:  Explained that we are going to identify vectors with affine entries zero with vectors in $\reals^{n-2}$ ant they're just going to have to live with that.
To avoid encumbering our notation with projection and inclusion maps, throughout this section we identify vectors in $\reals^n$ whose affine entries are zero with vectors in $\reals^{n-2}$.
In particular, we freely apply mutation maps $\eta^{\Comp(B)^T}$ to such vectors.
...
\item
%SinceSelecta:  This is the final form where we identify $\reals^{n-2}$ with affine-entries-zero.
If $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_k(x)$ are again zero and $\eta^{B^T}_k(x)=\eta^{\Comp(B)^T}_k(x)$.
%SinceSelecta:  Restated this with the notation for the projection.
%ACTUALLY, didn't do this!
%If $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_k(x)$ are again zero and $\bigl(\eta^{B^T}_k(x)\bigr)_\na=\eta^{\Comp(B)^T}_k(x_\na)$.
...
%SinceSelecta:  Restated this with the notation for the projection.
%ACTUALLY, didn't do this!
%Thus if $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_k(x)$ are zero and $\bigl(\eta^{B^T}_k(x)\bigr)_\na=\eta^{\Comp(B)^T}_k(x_\na)$.
%SinceSelecta:  Instead, we identify $\reals^{n-2}$ with affine-entries-zero.
Thus if $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_k(x)$ are zero and $\eta^{B^T}_k(x)=\eta^{\Comp(B)^T}_k(x)$.
...
\item
%SinceSelecta:  This was incorrect, because $\eta^{\Comp(B)^T}_k(x)$ is applying a mutation map in
%$\reals^{n-2}$ to a vector in $\reals^n$.  I made the same correction here that we had previously
%made in Proposition~\ref{nonspecial mut}.
%ACTUALLY, didn't do this!
%If $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_\kk(x)$ are again zero and $\bigl(\eta^{B^T}_\kk(x)\bigr)_\na=\eta^{\Comp(B)^T}_k(x_\na)$.
%SinceSelecta:  Instead, we identify $\reals^{n-2}$ with affine-entries-zero.
If $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_\kk(x)$ are again zero and $\eta^{B^T}_\kk(x)=\eta^{\Comp(B)^T}_k(x)$.
%If $x$ is a vector whose affine entries are zero, then $\eta^{B^T}_\kk(x)=\eta^{\Comp(B)^T}_k(x)$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
%SinceSelecta:  Fixed this to match the statement
...
We see the affine entries of $\eta^{B^T}_\kk(x)$ are zero and that $\eta^{B^T}_\kk(x)=\eta^{\Comp(B)^T}_k(x)$.
...
\item
%SinceSelecta:  This was incorrect, because it was phrased in the erroneous way that I just fixed in
%Proposition~\ref{special mut}.
%ACTUALLY, the fix is to identify $\reals^{n-2}$ with affine-entries-zero.
if $x$ is a vector whose affine entries are zero, then the affine entries of $\eta^{B^T}_{\widehat\kk}(x)$ are zero and $\eta^{B^T}_{\widehat\kk}(x)=\eta^{\Comp(B)^T}_\kk(x)$.
...
Substituting into $\eta^{\BB_0^T}_\kk(-\frac12\tB_0\delta^{B_0})=-\frac12\tB\delta^B$, we obtain $\eta^{\BB_0^T}_\kk\big(\eta^{\BB^T}_{(\kk')^{-1}}(-\frac12\tB'\delta^{B'})\big)=-\frac12\tB\delta^B$, or in other words $\eta^{\BB^T}_{\kk(\kk')^{-1}}(-\frac12\tB'\delta^{B'})=-\frac12\tB\delta^B$, as desired.
\end{proof}
