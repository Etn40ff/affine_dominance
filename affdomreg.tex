...
Then there exists a $c$-sortable element~$v$ whose associated seed $t$ has exchange matrix $B$.  \sayN{Have we explained ``associated seed''?  (I guess, sort of, above, but only for finite type.)  I think we mean the seed at the end of the green sequence in Proposition~\ref{sort green}, but we should say so.}
