...
\begin{document}
  ...
  For $\tilde\lambda\in\RR^m$ and $\bfk=(k_1,\ldots,k_r)$, define $S_{\bfk,\tilde\lambda}=\left\{\left(\eta^{\tilde{B}}_\bfk\right)^{-1}\big(\eta^{\tilde B^{t_+}}_\bfk(\tilde\lambda)+\tilde{B}^{t_+}\alpha\big):\alpha\in\RR^n_{\ge0}\right\}$.
    ...
    \begin{itemize}
      ...
      \item $\cP(\tilde\lambda)$ contains $\{\tilde\lambda+r\tilde B^+\cdot\delta:0\le r\le s\}$.
        \begin{itemize}
          ...
          \item for $t'$ connected to $t$ by mutation in direction $k$, $\eta_k^{\tilde B^t}(\tilde B^t\cdot\kappa_t)=\tilde B^{t'}\cdot\kappa_{t'}$
        \end{itemize}
