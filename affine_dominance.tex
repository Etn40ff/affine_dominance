...
\begin{document}
  ...
  Given a sequence $\bfk=(k_1,\ldots,k_r)$ with $k_i\in\{1,\ldots,n\}$ write $\mu_\bfk$ for the iterated matrix mutation $\mu_{k_r}\circ\cdots\circ\mu_{k_1}$.
  ...
  \end{itemize}
  More directly, when $t'$ is obtained from $t$ by following edges labeled by $\bfk=(k_1,\ldots,k_r)$, we have $\tilde B^{t'}=\mu_\bfk \tilde B$.
  ...
  Given $\bfk=(k_1,\ldots,k_r)$ and any $\tilde B$, define the piecewise-linear \emph{mutation map} $\eta^{\tilde B}_\bfk:\RR^m\to\RR^m$ where $\eta^{\tilde B}_\bfk(\nu)$ is the last column of $\mu_\bfk([\tilde{B}\;\nu])$.
  ...
  \begin{itemize}
    ...
    \item A maximal (by inclusion) cone $K$ in $\cF_{B^+}$ is \emph{real} if there exists $t\in\TT_n$ such that $K=G^t\cdot\RR^n_{\ge0}$, otherwise it is \emph{imaginary}.
  ...
  \begin{definition}
    Green cone $\Lambda_\lambda^+$ and red cone $\Lambda_\lambda^-$
  \end{definition}

  Define $\Lambda^\pm_\lambda:=\lim_{k\to\infty} \Lambda^\pm_{\lambda_k}$.
