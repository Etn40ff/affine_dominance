...
\begin{document}
  ...
  \begin{theorem}
    \label{thm:main}
    Let $\lambda$ be imaginary \saySS{define imaginary, real, and the imaginary cone $\cI$, $\widetilde B^+$, $\delta$} with $z$ minimal such that $\lambda+z\widetilde B^+\delta$ is real.
    \saySS{We need to say why such an $z$ existsi, i.e. $-\widetilde B^+\delta$ positively spans the imaginary ray.}
    Then the dominance region $\cP_\lambda$ is the segment $\{\lambda+x\widetilde B^+\delta:0\le x\le z\}$.
  \end{theorem}

  The proof of Theorem~\ref{thm:main} is divided into several intermediate claims; we begin by showing that $\cP_\lambda \subseteq \cI$.
  ...
  \begin{proof}
    ...
    In particular, for sufficiently large $z$, the dominance region $\cP_\lambda$ does not contain a small enough tubular neighbourhood $U$ of the half-line $\{-x\widetilde B^+\delta: x\ge z\}$.
  Suppose by contraddiction that there exists some point $\lambda' \in \cP_\lambda\setminus\, \cI$.
  Since $\cP_\lambda$ is stable under mutations $\eta_{\bfk}^{\widetilde B^+}(\lambda')\in \cP_\lambda$ for any for any sequence of indices $\bfk$.
  \saySS{Define mutation maps. of course we already knew this was to be done!}
  Moreover, since $\lambda'$ is not in $\cI$, for sufficiently large $\ell$ the sequence $\bfk = (n,\dots,1)^\ell$ satisfies $\eta_{\bfk}^{\widetilde B^+}(\lambda')\in U$ contraddicting our previous observation.
  \saySS{Do we want to say more here concerning the limiting behaviour along coxeter mutations?}
  \end{proof}
  ...
  From Proposition~\ref{prop:ray upperbound}, from the fact that $\cP_\lambda\subseteq \{\lambda + \widetilde B^+ \alpha : \alpha \in  \RR_{\ge0}^n\}$, and from the fact that $\widetilde B^+$ is full rank  we deduce immediately the following upper bound for $\cP_\lambda$.
  \begin{corollary}
  ...
  \begin{proposition}
    Let $\lambda$ be imaginary with $z$ minimal such that $\lambda+z\widetilde B^+\delta$ is real.
    Then $\cP_\lambda$ contains the segment $\{\lambda+x\widetilde B^+\delta:0\le x\le z\}$.
  \end{proposition}
  \begin{proof}
  \end{proof}
...
Below here is the stuff we wrote so far.
  ...
  \begin{lemma}
    Suppose $t$ is obtained from $t_+$ by mutating along the sequence $\bfk=(k_N,\ldots,k_1)$ passing through $t_+=t_1,\ldots,t_{N+1}=t$.
    Consider seeds $t'_1,\ldots,t'_{N+1}$ with $t'_{r+1}$ obtained from $t'_r$ by mutation in direction $k_r$ and $t'_1$ obtained from $t_+$ by mutating along the sequence $(n,\ldots,1)$ sufficiently many times that $\big(G^{\vee,t'_r}\big)^T \delta$ is a nonnegative vector for every $r$. 
    \saySS{This sentence needs to be justified; somewhere we need to discuss the plane $\delta^\perp$ and how it cuts the g-ector fan.}
    \sayDR{Think about $\vee$ or no $\vee$ in the $G$ matrix here.}
    For $0\le r\le N$, define $\delta_r = F^{t_r}_{k_r,-\varepsilon^{t'_r}_{k_r}} \cdots F^{t_1}_{k_1,-\varepsilon^{t'_1}_{k_1}} \cdot \delta$.
    Then the following hold.
    \begin{enumerate}
      \item $\delta_r$ is a nonnegative vector for $0\le r\le N$;
      \item For $1\le r\le N+1$, the sign of the $k_r$-th entry of $\widetilde B^{t_r} \delta_{r-1}$ is $\varepsilon^{t'_r}_{k_r}$.
      \item $\eta_\bfk^{\widetilde B^+}\left(-\widetilde B^+\delta\right)=-\widetilde B^t \delta_N$.
    \end{enumerate}
  \end{lemma}
  \begin{proof}
