...
\begin{document}
  ...
  \begin{lemma}
    If $\widetilde \lambda$ is imaginary then $\cP(\widetilde\lambda)\subseteq \cI$.
    \saySS{Define the imaginary cone $\cI$.}
  \end{lemma}
  \begin{proof}
    By definition $\cP(\widetilde\lambda)$ is contained in $\widetilde\lambda + \widetilde B^+ \cdot \RR_{\ge0}^n$ which is a proper cone since $\widetilde B^+$ is full rank.
    In particular, for sufficiently large $r$, $\cP(\widetilde\lambda)$ does not contain a tubular neighbourhood $U$ of the half-line $\{-s\widetilde B^+\delta: s\ge r\}$.
  Suppose by contraddiction that there exists some point $\widetilde \lambda' \in \cP(\widetilde\lambda)\setminus \cI$.
  Since $\cP(\widetilde\lambda)$ is stable under mutations $\eta_{\bfc^k}^{\widetilde B^+}(\widetilde\lambda')\in \cP(\widetilde\lambda)$ for any $k$.
  \saySS{Define $\bfc^k$ to be the sequence $n\dots,1,n,1,\dots$ of the appropriate lenght or find a better notation for this. Note that we might have to rethink the order in which we apply mutation maps.}
  On the other hand, since $\widetilde\lambda'$ is not in $\cI$, for sufficiently large $k$ we have $\eta_{\bfc^k}^{\widetilde B^+}(\widetilde\lambda')\in U$ contraddicting our previous observation.
  \end{proof}
