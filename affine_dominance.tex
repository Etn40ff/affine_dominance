...
\[
  \bfk_j = 
  \begin{cases}
    (1,\dots,n)^{-j} & \mbox{if } j<0\\
    ()               & \mbox{if } j=0\\
    (n,\dots,1)^{j}  & \mbox{if } j>0.\\
  \end{cases}
\]
Denote by $t_j$ the seed obtained from $t_+$ mutating along the sequence $\bfk_j$.
\begin{remark}
  If $j\geq 0$ the seed $t_j$ corresponds to the $c$-sorting word $c^j$.
  If $j<0$ the seed $t_j$ corresponds to the $c^{-1}$-sorting word $c^{j+1}$.
\end{remark}
Set $\cP_{\lambda,j}= \left(\eta^{\widetilde B^{t_+}}_{\bfk_j}\right)^{-1}\left(\left\{\eta^{\widetilde B^{t_+}}_{\bfk_j}(\lambda)+ \mu_{\bfk_j}(\widetilde B^{t_+})\alpha:\alpha\in\RR^n_{\ge0}\right\}\right)$; then $\cP_\lambda\subseteq \bigcap_{j\in\ZZ} \cP_{\lambda,j}$.

\begin{lemma}
  For any $j\geq 0$, there exists a full-dimensional subset $L_j$ of $\RR^m$ such that $\cI \subseteq L_j$ and, for any $\lambda\in\cI$,
  \[
    \cP_{\lambda,j}\cap L_j = \left\{\lambda + \widetilde B^{t_+} C^{t_j}\alpha:\alpha\in\RR^n_{\ge0}\right\} \cap L_j.
  \]
\end{lemma}
\begin{proof}
  Let $\bfl$ be any suffix of $\bfk_j$ and suppose that its first letter is $i$ ($0$ if $\bfl$ is empty).
  Then, counting modulo $n$, the $(i+1)$-st coordinate of $\eta^{\widetilde B^{t_+}}_{\bfl}(\lambda)$ is non-positive. 
  Indeed, since $\eta^{\widetilde B^{t_+}}_{\bfl}(\lambda)\in \mu_\bfl(\cI)$, it is of the form $M v$ for some non-negative vector $v$ and some matrix $M$ whose $(i+1)$-st row is a negative  elementary unit vector ($M$ is the matrix of the change of coordinates in between positive $\bfd$-vectors and non-initial $\bfg$-vectors).
  \saySS{I am trying very hard not to mention the Euler matrix here nor source-sink moves if possible}
  In particular $\eta^{\widetilde B^{t_+}}_{\bfk_j}$ acts on $\cI$ as $\widetilde E^{t_{nj-1}}_{n,-} \cdots \widetilde E^{t_0}_{1,-}$

  Let $L_j$ be the maximal cone on which $\eta^{\widetilde B^{t_+}}_{\bfk_j}$ and $\widetilde E^{t_{nj-1}}_{n,-} \cdots \widetilde E^{t_0}_{1,-}$ agree. 
  \saySS{Should we something to explain why this is not just $\cI$?}
  We compute
  \begin{align*}
    \cP_{\lambda,j}\cap L_j 
    & = \left(\eta^{\widetilde B^{t_+}}_{\bfk_j}\right)^{-1}\left(\left\{\eta^{\widetilde B^{t_+}}_{\bfk_j}(\lambda)+ \mu_{\bfk_j}(\widetilde B^{t_+})\alpha:\alpha\in\RR^n_{\ge0}\right\}\right) \cap L_j \\
    & = \left(\widetilde E^{t_{nj-1}}_{n,-} \cdots \widetilde E^{t_0}_{1,-}\right)^{-1}\left(\left\{\widetilde E^{t_{nj-1}}_{n,-} \cdots \widetilde E^{t_0}_{1,-}(\lambda)+ \mu_{\bfk_j}(\widetilde B^{t_+})\alpha:\alpha\in\RR^n_{\ge0}\right\}\right) \cap L_j \\
    & = \left\{\lambda + \widetilde E^{t_0}_{1,-} \cdots \widetilde E^{t_{nj-1}}_{n,-}\mu_{\bfk_j}(\widetilde B^{t_+})\alpha:\alpha\in\RR^n_{\ge0}\right\} \cap L_j \\
    & = \left\{\lambda + \widetilde B^{t_+} F^{t_0}_{1,-} \cdots F^{t_{nj-1}}_{n,-}\alpha:\alpha\in\RR^n_{\ge0}\right\} \cap L_j \\
    & = \left\{\lambda + \widetilde B^{t_+} C^{t_j}\alpha:\alpha\in\RR^n_{\ge0}\right\} \cap L_j
  \end{align*}
  where the last identity follows from Lemma~\ref{le:c-matrix factorization} and the observation that the $(i+1)$-st $\bfc$-vector in the seed mutating from $t^+$ along $\bfl$ is positive (it is the leftmost skip for the corresponding $c$-sorting word).
\end{proof}

\begin{lemma}
  For any $j< 0$, there exists a full-dimensional subset $L_j$ of $\RR^m$ such that $\cI \subseteq L_j$ and, for any $\lambda\in\cI$,
  \[
    \cP_{\lambda,j}\cap L_j = \left\{\lambda - \widetilde B^{t_+} C^{t_{\bfk_{j-1}}}\alpha:\alpha\in\RR^n_{\ge0}\right\} \cap L_j.
  \]
\end{lemma}
\hrule
...
\end{lemma}
\begin{proof}
  First observe that the $\langle j+1\rangle$-st coordinate of $\eta^{\widetilde B^{t_0}}_{\bfk_j}(\lambda)$ is non-positive for every $j\geq 0$.
  Indeed, since $\eta^{\widetilde B^{t_0}}_{\bfk_j}(\lambda)\in \mu_{\bfk_j}(\cI)$, it is of the form $M v$ for some non-negative vector $v$ and some matrix $M$ whose $\langle j+1\rangle$-st row is a negative  elementary unit vector ($M$ is the matrix of the change of coordinates in between positive $\bfd$-vectors and non-initial $\bfg$-vectors).
  \saySS{I am trying very hard not to mention the Euler matrix here nor source-sink moves if possible}
  ...
  \begin{align*}
    ...
    & = \left\{\lambda + \widetilde E^{t_0}_{1,-} \cdots \widetilde E^{t_{j-1}}_{\langle j \rangle,-}\mu_{\bfk_j}(\widetilde B^{t_+})\alpha:\alpha\in\RR^n_{\ge0}\right\} \cap L_j \\
  ...
  \end{align*}
  where the last identity follows from Lemma~\ref{le:c-matrix factorization} and the observation that the $\langle j+1\rangle$-st $\bfc$-vector in the seed $t_j$ is positive (it is the leftmost skip for the corresponding $c$-sorting word).
\end{proof}
