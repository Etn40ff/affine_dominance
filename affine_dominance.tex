...
\saySS{Note to self: if $\lambda$ is a cluster monomial in $t$ then exactly the same argument shows that $\cP_{\lambda,j}\cap K_t$ has the same expression. This should imply that $\cP_\lambda\cap K_t$ is just the point $\lambda$ but it is not enough to conclude that $\cP_\lambda=\lambda$ because we do not know that $\cP_\lambda$ is connected.}
  ...
  By direct inspection we see that the only possibilities are then those in Table~\ref{tab:possible_submatrices}.
	Note that, as the sign patterns are the same regardless of the chose tube mutating in directions $n-1$ and $n$ do not affect the exchange matrix at $t$ outside of rows and columns $n-1$ and $t$.
  ...
  To summarize the exchange matrix at $t$, after relabelling, is of the form
    \[
      ...
      \right]
    \] 
    with $B_{11}, B_{22}, B_{33}$ of finite type $A$, $B_{i4}$ non zero only in the last row, $B_{4j}$ non zero only in the last columns, and such that the non-zero rows and columns in 
    \[
      \left[
      \begin{array}{cc} 
        0 & B_{i4} \\ 
