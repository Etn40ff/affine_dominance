...
\begin{document}
  ...
  \begin{proof}
    ...
    In particular, for sufficiently large $z$, the dominance region $\cP_\lambda$ does not intersect the halfspace $H = \{ \lambda' : \langle -B^+\delta, \lambda' \rangle \ge z \}$.
  ...
  Moreover, since $\lambda'$ is not in $\cI$, for sufficiently large $\ell$ the sequence $\bfk = (n,\dots,1)^\ell$ satisfies $\eta_{\bfk}^{\widetilde B^+}(\lambda')\in H$ contraddicting our previous observation.
\saySS{We need to say much more about this: explain finite/infinite c-orbits and how limits work.}
  ...
  \begin{proposition}
    ...
    \saySS{Change this statement to hide Cambrian stuff} 
  \end{proposition}
  \begin{proof}
  ...
  \begin{proof}
    ...
    By assumption the root $\beta$ is in a finite $c$-orbit \saySS{again explain c-orbits} so that there is a positive $\ell$ such that $\sum_{i=0}^\ell c^i \beta = q \delta$ for some $q\neq 0$ (cf. \cite{afforb}).
  ...
  \begin{proof}
    ...
    Redefining the seeds $t_i'$, if needed, to ensure that the hypotheses of Proposition~\ref{prop:no postinjectives} are satisfied and using Lemma~\ref{le:euler pairing} we see that the $k_i$-th sign of $B^{t'_i} \delta_i$ weakly agrees with the sign of the $k_i$-th column of $C^{\vee,t'_i}$.
