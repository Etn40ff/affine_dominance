...
\saySS{Note to self: if $\lambda$ is a cluster monomial in $t$ then exactly the same argument shows that $\cP_{\lambda,j}\cap K_t$ has the same expression. This should imply that $\cP_\lambda\cap K_t$ is just the point $\lambda$ but it is not enough to conclude that $\cP_\lambda=\lambda$ because we do not know that $\cP_\lambda$ is connected.}
  ...
  \begin{proof}
    ...
    In that case, $F_{k_N,+}^{t_N}\delta_N=F_{k_N,-}^{t_N}\delta_N=\delta_{N+1}$ since the $k_N$-th row of $-B^{t_N}$ is equal to the $k_N$-th row of $F_{k_N,+}^{t_N}-F_{k_N,-}^{t_N}$.
    It follows that the equality $\langle\delta_N,\lambda-\mu\rangle=0$ mutates to the equality $\langle\delta_{N+1},\lambda-\mu\rangle=0$.
    \sayDR{This is only immediately true in the symmetric case where $(E_{k_N,\pm}^{t_N})^T=F_{k_N,\mp}^{t_N}$, otherwise some manipulations with symmetrizers is necessary to relate $(E_{k_N,pm}^{t_N})^T$ to $F_{k_N,\mp}^{t_N}$.}
  \end{proof}
